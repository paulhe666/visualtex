...
try {
    ...
    if($targetDoc.Content.Text -ne $sourceText){throw 'Target stage text does not exactly match the source document after copy.'}
    $targetDoc.Activate()
    ...
    Write-Output ("SOURCE_COPIED|sourcePid=$SourceWordProcessId|source=$SourceDocumentName|targetPid=$($record.pid)|target=$($targetDoc.Name)|length=$($sourceText.Length)")
}
finally {
