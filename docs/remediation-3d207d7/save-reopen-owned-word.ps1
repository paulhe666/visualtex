param([Parameter(Mandatory=$true)][int]$WordProcessId,[Parameter(Mandatory=$true)][string]$ExpectedName,[Parameter(Mandatory=$true)][string]$Label)
...
if(Test-Path -LiteralPath $path){throw 'Evidence file already exists; refusing overwrite.'}
$word=Connect-RunningWord $WordProcessId
$document=$word.ActiveDocument
if($document.Name -ne $ExpectedName -or $document.Path){throw 'Active document is not the explicitly named new stage document.'}
...
$document.SaveAs2([string]$path,12)
