...
7. Switch presentations before saving. VisualTeX must reject the write and leave both presentations unchanged. Repeat a commit after forcing the Session completion write to fail: PowerPoint must recognize the final SessionId/metadata/geometry object and must not create a second shape.
