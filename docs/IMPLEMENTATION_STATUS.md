...
| 4. Bidirectional structured editing | Substantially complete | CodeMirror and the real compiled-page editor share one revisioned Core buffer; VisualTeX 1.0.6-derived MathLive workbench supports command candidates, blue selection styling, selection-aware insert/replace, matrices, clear/delete and one authoritative edit surface; paragraphs support text addition/removal/replacement; all changes use Core undo/redo and UTF-8 byte spans | Rich clipboard schema, larger IME/e2e matrix, title/author projection through `maketitle` and broader arbitrary-macro coverage |
...
| 6. PDF preview and SyncTeX | Substantially complete | PDFium document inspection, page/tile and thumbnail rendering, fingerprinted cache, Windows-safe atomic PNG publication, pixel comparison, desktop canvas viewer, VS Code PDF viewer, forward/inverse SyncTeX, and PDFium character/font/bounds extraction that distinguishes prose from inline formulas even when SyncTeX returns identical boxes and no column | Rotated-page glyph-coordinate validation, larger-document virtualization/performance profiling and native three-platform PDFium release validation |
...
| 8. Direct PDF-page editing | Functional baseline | The desktop “结构化编辑” tab uses the actual compiled PDF page; SyncTeX, source spans and PDFium glyph sequences separate ordinary text from inline formulas even when SyncTeX omits columns or returns the wrong line; corroborated medium-confidence paragraphs are editable; paragraph text is recovered into filtered PDF visual lines with one input and one page outline per line; a viewport-clamped floating workbench avoids page clipping and duplicate inspector editors; formulas and paragraphs can be edited or deleted; figures support path, `\\linewidth` width slider/presets, page resize handle, float placement, caption and label | Reflow relocation across large edits, complex custom-macro hit testing, title/author mapping through `maketitle`, image asset browsing/cropping and broader node coverage |
...
| 15. Release and stability | Partial | Offline defaults, recovery, model package verification, dark/light UI, CI checks, `visualstudio` 0.1.3 debug/release builds, unsigned Windows x64 NSIS installer smoke workflow, security documentation, real Chinese PDF glyph/visual-line verification and a compiled text/formula/image visual-edit fixture with zero authoritative-vs-shadow pixel difference | Authenticode signing, macOS notarization, Linux/macOS installers, updater verification, accessibility/i18n audit, memory/leak profiling, crash telemetry policy and release regression matrix |
...
- Real PDFium glyph checks distinguish `CMMI10` formula glyphs from SimSun Chinese prose on the same visual line while SyncTeX reports no column; medium-confidence paragraphs are recovered into ordered visual lines and drifting rectangles from the following paragraph are filtered by source content.
- `fixtures/visual-edit` compiles paragraphs, two inline formulas, a display equation and an `includegraphics` figure; its authoritative and shadow PDFs have zero changed pixels.
- Unsigned `visualstudio` 0.1.3 Windows x64 NSIS installer build plus silent install, `visualstudio.exe` version/process/window-title launch check and uninstall smoke test.
