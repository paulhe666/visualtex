...
| Windows x64 | [VisualTeX_1.1.0_x64-setup.exe](https://github.com/paulhe666/visualtex/releases/download/v1.1.0/VisualTeX_1.1.0_x64-setup.exe) | NSIS 安装程序，可一键配置 Word/PowerPoint 集成（Office.js + 本地 COM/OLE 自动化桥） |
...
- PowerPoint 支持按钮编辑和双击编辑；公式以 PNG 图片形状写入，并在 Shape 标签和替代文本中保存可恢复的 VisualTeX metadata；
...
### Windows Office 集成（Office.js + COM/OLE 自动化桥）

- Windows 安装器中的选项名称为“VisualTeX + OLE Office 集成”；这里的 OLE 指本地伴侣服务通过 Office COM/OLE 自动化控制 Word 和 PowerPoint，并不是把公式作为 OLE 嵌入对象写入文档；
- Word 中的公式实际保存为带 VisualTeX 元数据的 `InlineShape` PNG 图片，PowerPoint 中保存为带标签和替代文本的 `Shape` PNG 图片；
- 勾选后自动完成证书、可信 Office Catalog、Word/PowerPoint Manifest、Ribbon 缓存、后台启动和旧 VSTO 禁用；
- 正常安装成功后无需额外手动注册；安装时应先关闭 Word 和 PowerPoint；
- 设置页可修复 Windows Office 集成、停止当前伴侣服务、关闭开机启动或移除 Office Manifest。
...
- Windows 教程说明 Office.js + 本地 COM/OLE 自动化桥配置完成后的状态，以及如何停止服务、关闭开机启动和移除 Manifest；
...
5. 若安装器提示 Office 配置失败，请打开 VisualTeX 设置并点击“修复 OLE 集成”（界面沿用该名称）。
...
- MathJax：SVG 公式导出；
- Microsoft Office.js + macOS AppleScript + Windows COM/OLE 自动化桥：Word/PowerPoint 集成；公式内容以图片形状和 VisualTeX 元数据保存，不是 OLE 嵌入对象；
...
| Windows x64 | [VisualTeX_1.1.0_x64-setup.exe](https://github.com/paulhe666/visualtex/releases/download/v1.1.0/VisualTeX_1.1.0_x64-setup.exe) | NSIS installer with one-step Word/PowerPoint setup using Office.js and a local COM/OLE automation bridge |
...
- PowerPoint button and double-click editing; formulas are stored as PNG picture shapes with durable VisualTeX metadata in shape tags and alternative text;
...
### Windows Office integration (Office.js + COM/OLE automation bridge)

- The installer option is named **VisualTeX + OLE Office integration**. Here, OLE refers to the local companion using Office COM/OLE automation; formulas are not embedded as OLE objects;
- Word formulas are stored as PNG `InlineShape` pictures carrying VisualTeX metadata, while PowerPoint formulas are PNG `Shape` pictures with metadata in tags and alternative text;
...
- Settings can repair the Windows Office integration, stop the current companion, disable startup, or remove the Office manifest.
...
5. If setup reports an Office integration failure, open VisualTeX Settings and choose **Repair OLE integration** (the UI keeps this existing label).
