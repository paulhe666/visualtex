...
export const VISUALTEX_PHYSICS_KERNEL_MACROS: readonly VisualTexPhysicsKernelMacro[] =
  Object.freeze([
    ...
    entry("svmqty", "\\left|\\begin{smallmatrix}#1\\end{smallmatrix}\\right|", 1),
    entry("smallmatrixquantity", "\\begin{smallmatrix}#1\\end{smallmatrix}", 1),
    entry("smqty", "\\begin{smallmatrix}#1\\end{smallmatrix}", 1),
    entry("smallmatrixdeterminant", "\\left|\\begin{smallmatrix}#1\\end{smallmatrix}\\right|", 1),
    entry("smdet", "\\left|\\begin{smallmatrix}#1\\end{smallmatrix}\\right|", 1),
    entry("absolutevalue", "\\left|#1\\right|", 1),
    ...
    entry("outerproduct", "\\left|#1\\right\\rangle\\!\\left\\langle#2\\right|", 2),
    entry("dyad", "\\left|#1\\right\\rangle\\!\\left\\langle#2\\right|", 2),
    entry("op", "\\left|#1\\right\\rangle\\!\\left\\langle#2\\right|", 2),
    entry("expectationvalue", "\\left\\langle#2\\right|#1\\left|#2\\right\\rangle", 2),
    entry("ev", "\\left\\langle#1\\right\\rangle", 1),
    entry("vev", "\\left\\langle0\\right|#1\\left|0\\right\\rangle", 1),
    entry("matrixelement", "\\left\\langle#1\\right|#2\\left|#3\\right\\rangle", 3),
    entry("matrixel", "\\left\\langle#1\\right|#2\\left|#3\\right\\rangle", 3),
    entry("flatfrac", "#1/#2", 2),
  ]);
