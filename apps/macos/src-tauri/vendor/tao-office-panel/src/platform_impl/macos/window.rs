...
fn create_window(
  ...
  unsafe {
    ...
    // Office formula windows must be born as real AppKit panels. Turning an
    // existing NSWindow into a "nonactivating panel" later does not let it
    // become key while another application owns the foreground.
    let office_panel = attrs.title == "VisualTeX Office Formula";
    if office_panel {
      masks |= NSWindowStyleMask::NonactivatingPanel;
    }
    let window_class = if office_panel { OFFICE_PANEL_CLASS.0 } else { WINDOW_CLASS.0 };
...
fn register_window_class(
  name: &'static CStr,
  window_superclass: &'static Class,
  panel: bool,
) -> WindowClass {
  unsafe {
    let mut decl = ClassDecl::new(name, window_superclass).unwrap();
    decl.add_method(
      sel!(canBecomeMainWindow),
      if panel { is_never_main } else { is_focusable } as extern "C" fn(_, _) -> _,
    );
    decl.add_method(
...
static WINDOW_CLASS: Lazy<WindowClass> = Lazy::new(|| {
...
static OFFICE_PANEL_CLASS: Lazy<WindowClass> = Lazy::new(|| {
  register_window_class(
    CStr::from_bytes_with_nul(b"TaoOfficePanel\0").unwrap(),
    class!(NSPanel),
    true,
  )
});

extern "C" fn is_never_main(_: &Object, _: Sel) -> Bool {
  NO
}
