...
expectIncludes(wordAdapter, '"word-events-external-seq-safe-insert-20260722-r31"', "Word health must identify the reviewed image/native external-SEQ, stable-reconciliation, deleted-formula cleanup, image identity restoration and baseline-center VBA revision");
...
expectIncludes(wordAdapter, "Private Function VTCalibrateImageEquationNumberPosition", "Image Equation numbers must use a real-host page-coordinate calibration loop");
expectIncludes(wordAdapter, "nextPosition = currentPosition - CLng(residual)", "Image-number calibration must preserve measured baseline feedback instead of applying a fixed raise");
expectIncludes(wordAdapter, "formulaCenterY = formulaY - formulaShape.Height / 2!", "Image-number calibration must treat Word's InlineShape coordinate as its baseline/bottom anchor");
expectIncludes(wordAdapter, "numberCenterY = numberY - numberLineHeight / 2! - currentPosition", "Image-number calibration must derive the visible number center upward from its baseline and Font.Position");
expectIncludes(wordAdapter, "numberCenterY = numberY - numberLineHeight / 2! - numberPosition", "The independent image layout assertion must use the same baseline-derived number center as calibration");
expectIncludes(wordAdapter, "Abs(formulaCenterY - numberCenterY) > 2!", "Image-number calibration must reject a remaining visual-center error larger than two points");
...
expectIncludes(wordAdapter, 'regressionStage = "refresh-fields-and-repaginate"', "The dedicated image-number regression must repeat baseline measurements after field refresh and repagination");
...
expectIncludes(wordAdapter, '"centerErrorAfter=" & _', "The dedicated image-number PASS report must record the post-refresh visual-center error");
...
expectIncludes(wordAdapter, "Set sequenceField = VTFindEquationSequenceField(paragraphRange)", "New visible Equation numbers must remain live native SEQ fields in the formula paragraph");
...
expectIncludes(wordAdapter, "VTEquationNumberRaisePoints = 0!", "Image numbering must start from a neutral baseline before real-host calibration");
expectIncludes(wordAdapter, "numberPosition = VTCalibrateImageEquationNumberPosition", "Renumbering must recalibrate the actual image-number visual axis after every field refresh");
...
expectIncludes(wordAdapter, "Set sequenceField = VTFindEquationSequenceField(paragraphRange)", "Existing single-paragraph numbered formulas must reuse and refresh their native SEQ field");
...
expectIncludes(packager, "word-events-external-seq-safe-insert-20260722-r31", "Packaging must reject a Word DOTM that predates pure-number native image references, image identity restoration, helper cleanup and stable field reconciliation");
