...
assert.deepEqual(
  splitOcrLatexIntoFormulaLines(String.raw`\[a+b\\c+d\]`),
  ["a+b", "c+d"],
...
assert.deepEqual(
  splitOcrLatexIntoFormulaLines(
    String.raw`\begin{align*}u&=v+w\\t&=\sqrt{x}\end{align*}`,
  ),
  [
    `u${VISUALTEX_ALIGNMENT_MARKER_LATEX}=v+w`,
    `t${VISUALTEX_ALIGNMENT_MARKER_LATEX}=\\sqrt{x}`,
  ],
  "align* must unwrap and encode only top-level alignment anchors",
);

console.log("macOS OCR multiline formula-row normalization passed.");
