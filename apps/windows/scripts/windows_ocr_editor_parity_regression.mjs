...
assert.ok(ocrService.includes("setActiveOcrProvider"));
assert.ok(ocrService.includes("OCR_RECOGNIZER_OPTIONS"));
assert.ok(ocrService.includes('id: "provider:openai-compatible"'));
assert.ok(ocrService.includes('id: "provider:paddleocr"'));
assert.ok(ocrService.includes('id: "provider:simpletex"'));
assert.ok(app.includes("handleOcrRecognizerChange"));
assert.ok(officeDialog.includes("handleOcrRecognizerChange"));
assert.ok(app.includes("setActiveOcrProvider(parsed.provider)"));
assert.ok(officeDialog.includes("setActiveOcrProvider(parsed.provider)"));
assert.ok(editorWorkspace.includes('aria-label={isEn ? "OCR recognizer" : "OCR 识别器"}'));
assert.ok(editorWorkspace.includes('label={isEn ? "Local models" : "本地模型"}'));
assert.ok(editorWorkspace.includes('label={isEn ? "OCR APIs" : "OCR API"}'));
assert.ok(!app.includes("await getOcrRuntimeStatus("));
assert.ok(!officeDialog.includes("await getOcrRuntimeStatus("));
assert.ok(tauriLib.includes("recognize_with_local_fallback"));
assert.ok(silentOcrNative.includes("recognize_with_local_fallback"));
assert.ok(!silentOcrNative.includes("runtime_status(app.clone(), false).await"));
