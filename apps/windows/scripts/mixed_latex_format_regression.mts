...
assert.match(definition.descriptionZh, /Alt\+回车/);
assert.equal(
  resolveNewFormulaLineMode("mixed-inline-display", "display", {
    shiftKey: true,
    altKey: false,
  }),
  "inline",
...
assert.equal(
  resolveNewFormulaLineMode("mixed-inline-display", "inline", {
    shiftKey: false,
    altKey: true,
  }),
  "display",
  "Alt+Enter must create a display row in mixed mode",
);
assert.equal(
  resolveNewFormulaLineMode("mixed-inline-display", "inline", {
    shiftKey: false,
...
assert.equal(
  resolveNewFormulaLineMode("display-dollar", "inline", {
    shiftKey: true,
    altKey: false,
  }),
  undefined,
...
const textOnlyParsed = parseLatexSourceDraft(
  textOnlyInline,
  "mixed-inline-display",
);
...
assert.equal(
  legacyLine,
  "$$E=mc^2$$",
  "legacy rows must default to display mode",
);
