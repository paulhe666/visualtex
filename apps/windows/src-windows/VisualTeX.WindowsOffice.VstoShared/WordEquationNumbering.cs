...
{
    // A table cell is already a paragraph; a lone OLE is not evidence of
    // display mode. Require the native display tab scaffold as well.
    internal static bool IsTableMathTypeDisplayLayout(
        string? shapePrefixText,
        bool hasCenterTab,
        bool hasRightTab) =>
        string.Equals(shapePrefixText, "\t", StringComparison.Ordinal)
        && hasCenterTab
        && hasRightTab;
    ...
    // Word bookmark names are limited to 40 characters. Frames.Add can create one
    // extra body-flow paragraph after an external caption immediately before a
    // user table; this 7-character prefix + 32-character UUID owns only that exact
    // generated empty paragraph so reverse conversion can reclaim it safely.
    private const string NativeCaptionFlowTailBookmarkPrefix = "VTEqFT_";
    ...
    private static bool IsCustomUndoRecordActive(Document document)
    {
        UndoRecord? record = null;
        try
        {
            record = document.Application.UndoRecord;
            return record.IsRecordingCustomRecord || record.CustomRecordLevel > 0;
        }
        catch
        {
            return false;
        }
        finally { Release(record); }
    }
    ...
    internal sealed class EquationNumberFormatVariableSnapshot
    {
        internal EquationNumberFormatVariableSnapshot(bool exists, string value)
        {
            Exists = exists;
            Value = value;
        }
    ...
    internal static EquationNumberFormatVariableSnapshot CaptureEquationNumberFormatVariable(
        Document document)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                return new EquationNumberFormatVariableSnapshot(
                    exists: true,
    ...
    internal static void RestoreEquationNumberFormatVariable(
        Document document,
        EquationNumberFormatVariableSnapshot snapshot)
    {
        if (snapshot.Exists)
        {
            WriteEquationNumberFormat(document, snapshot.Value);
            return;
        }
        ...
        try
        {
            ...
            {
                // The absent pre-edit state has already been restored.
            }
        }
        finally
        {
    ...
    internal static bool NeedsLegacyManagedNumberingMigration(Document document)
    {
        ...
        try
        {
            ...
            var xml = WordDocumentXml.Read(document) ?? string.Empty;
            if (Regex.IsMatch(
                    xml,
    ...
    internal static int UpdateEquationNumbers(
        Document document,
        IReadOnlyCollection<string>? expectedNumberedFormulaIds = null)
    {
        ...
        var watch = tracePerformance
            ...
            : null;
        // A complete existing scaffold says nothing about newly inserted formulas.
        // Batch callers already know their required identities; the explicit Ribbon
        // command obtains the managed OMML set once from persisted metadata.
        var requiredIds = expectedNumberedFormulaIds ?? WordOmmlFormulaStore.StoredFormulaIds(document)
            .Where(id =>
            {
                var metadata = WordOmmlFormulaStore.TryRead(document, id);
                return metadata?.Numbered == true && metadata.DisplayMode == "block";
            }).ToArray();
        ...
        var fastPath = TryRefreshHealthyEquationNumbersInPlace(document, out var updated, requiredIds);
        if (!fastPath)
        {
            RepairNumberedDisplaySpacing(document);
            fastPath = TryRefreshHealthyEquationNumbersInPlace(document, out updated, requiredIds);
        }
        // An XML-proven document with no VisualTeX objects, aliases or references
        // has nothing for this numbering engine. MathType is refreshed separately.
        if (fastPath && updated == 0 && requiredIds.Count == 0) return 0;
        var result = fastPath ? updated : Reconcile(document);
        if (!fastPath && requiredIds.Count > 0
            && !TryRefreshHealthyEquationNumbersInPlace(document, out result, requiredIds))
            throw new InvalidDataException("Numbering did not create a complete host for every required formula.");
        // References retain their original names across format conversions.
        // The numeric-identity inventory alone cannot enumerate inherited ZEqnNum
        // references. Finalize both direct and nested references by their actual
        // target bookmarks, after either numbering implementation completes.
        // The healthy fast path already updates changed target references and
        // repairs every navigable REF discovered by its OpenXML inventory. A
        // second document-wide Fields scan here was pure duplicate work in large
        // sparse documents. Reconciliation does not share that inventory, so it
        // retains the compatibility-wide refresh.
        if (!fastPath)
    ...
    private static bool TryRefreshHealthyEquationNumbersInPlace(
        Document document,
        out int updated,
        IReadOnlyCollection<string>? requiredFormulaIds = null,
        bool deferExternalReferences = false)
    {
        updated = 0;
        var tracePerformance = string.Equals(
            Environment.GetEnvironmentVariable("VISUALTEX_NUMBERED_PERF_TRACE"),
        ...
        if (requiredFormulaIds is not null)
        {
            var materialized = new HashSet<string>(openXmlCaptions.Select(entry => entry.FormulaId),
                StringComparer.OrdinalIgnoreCase);
            if (requiredFormulaIds.Any(id => !materialized.Contains(id))) return false;
        }

        if (openXmlCaptions.Count == 0) return true;
        ...
        if (changedFormulaNumbers.Count > 0 && !deferExternalReferences)
        {
        ...
        // Recreated targets can retain the same number while their body REF still
        // contains Word's missing-bookmark result. REF health is independent of
        // whether a sequence value changed. Reuse the existing XML inventory to
        // avoid opening the Fields collection in documents with no body references.
        var referencedBookmarks = new HashSet<string>(referenceCounts
            .Where(entry => entry.Value > 1)
            .Select(entry => NativeNumberBookmarkName(entry.Key)),
            StringComparer.OrdinalIgnoreCase);
        if (referencedBookmarks.Count > 0 && !deferExternalReferences)
            WordEquationReferenceFields.UpdateNavigableReferences(document, referencedBookmarks);
        ...
        IReadOnlyCollection<string>? requiredFormulaIds = null)
    {
        ...
        try
        {
            ...
            TraceStage("caption-inventory");
            if (captions.Count == 0)
                return requiredFormulaIds is null || requiredFormulaIds.Count == 0;
            ...
            if (!TryApplyEquationNumberFormatByFieldBatch(
                    ...
                    deferExternalReferences: true))
                return false;
            TraceStage("field-batch");

            if (!TryRefreshHealthyEquationNumbersInPlace(document, out updated, requiredFormulaIds, deferExternalReferences: true))
                return false;
        ...
        {
            TraceNumberingPerformance($"[perf] conversion-numbering-finalize.failed error={error}");
            throw;
        }
    }
    ...
    internal static int SetEquationNumberFormat(Document document, string formatId)
    {
        var acceptanceTiming = string.Equals(
            ...
            ? System.Diagnostics.Stopwatch.StartNew()
            : null;
        WriteEquationNumberFormat(document, EquationNumberFormat.Resolve(formatId).Id);
        ...
        bool deferExternalReferences = false)
    {
        ...
        try
        {
            ...
            var hasOnlyGeneratedReferences = knownReferenceCounts is not null
                ...
                : HasOnlyGeneratedEquationReferences(
                    document,
                    captions.Count);
            if (!generatedReferencesPatched && deferExternalReferences)
                throw new InvalidDataException("The conversion could not update its own visible equation numbers.");
            if ((!generatedReferencesPatched || !hasOnlyGeneratedReferences) && !deferExternalReferences)
                UpdateNativeCrossReferences(document);
        ...
        {
            TraceNumberingPerformance($"[perf] equation-number-field-batch.failed mutationStarted={mutationStarted} error={error}");
            if (!mutationStarted)
            {
                updated = 0;
                return false;
            }
            // The enclosing Word transaction owns recovery. A second full
            // structural reconstruction would hide the original partial failure.
            throw;
        }
    ...
    internal static bool IsUnmanagedEquationDocumentXml(string xml)
    {
        ...
        // Absence of number aliases alone is not enough: an unnumbered/copied
        // VisualTeX object may still need adoption. Be conservative for all known
        // current and legacy metadata/reference spellings.
        if (xml.IndexOf("VTEq", StringComparison.OrdinalIgnoreCase) >= 0
            || xml.IndexOf("VTO_", StringComparison.OrdinalIgnoreCase) >= 0
            || xml.IndexOf("VTOMML_", StringComparison.OrdinalIgnoreCase) >= 0
            || xml.IndexOf("visualtex:", StringComparison.OrdinalIgnoreCase) >= 0
            || xml.IndexOf("VisualTeXEquation", StringComparison.OrdinalIgnoreCase) >= 0)
            return false;
        try
        {
            var parsed = XDocument.Parse(xml);
            var elements = parsed.Descendants().ToArray();
            return elements.Any(e => e.Name.LocalName == "body")
                && !elements.Any(e => e.Name.LocalName is "oMath" or "oMathPara")
                && !elements.Attributes().Any(a => a.Name.LocalName == "ProgID"
                    && a.Value.StartsWith("VisualTeX", StringComparison.OrdinalIgnoreCase));
        }
        catch { return false; }
    }
        ...
        out IReadOnlyDictionary<string, int> referenceCounts)
    {
        ...
        {
            content = document.Content;
            xml = WordDocumentXml.Read(document) ?? string.Empty;
            ...
            var bookmarkMatches = Regex.Matches(
                ...
                RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
            if (bookmarkMatches.Count == 0)
            {
                if (IsUnmanagedEquationDocumentXml(xml)) return true;
                return Fail("no-numbering-bookmarks");
            }
            if (Regex.IsMatch(
                    xml,
            ...
            foreach (var visibleStart in visibleStartMatches)
            {
                ...
                if (visibleNumberIsInMath)
                {
                    ...
                    var hashPrecedingTableEnd = xml.LastIndexOf(
                        ...
                        hashParagraphStart,
                        StringComparison.OrdinalIgnoreCase);
                    // The owning display paragraph may be inside a user cell.
                    // The checks above already prove the equation, native number,
                    // and all three aliases belong to one OMath in that paragraph.
                    // A table ancestor is not a different numbering format.
                    if (hashParagraphXml.IndexOf(
                ...
                var localParagraphStart = LastElementStart("w:p", visibleStart.Index);
                var localParagraphEnd = xml.IndexOf("</w:p>", visibleStart.Index,
                    StringComparison.OrdinalIgnoreCase);
                var cellOleNumber = visibleNumberIsInTable && localParagraphStart >= 0
                    && localParagraphEnd > visibleStart.Index
                    && IsOleNumberingParagraphXml(xml.Substring(localParagraphStart,
                        localParagraphEnd + "</w:p>".Length - localParagraphStart), formulaId);
                if (visibleNumberIsInTable && !cellOleNumber)
                {
                    ...
                    if (rowCount > 1)
                    {
                        // Current dense numbered-OMML tables may legitimately
                        // share one N x 3 host, but every row then owns exactly one
                        // center OMath. Historical Word 2019 insertions could leave
                        // an additional persisted 3-cell row containing no formula.
                        // Do not call that host healthy: failing the XML-only fast
                        // probe routes Update Numbers through the targeted spacing
                        // repair, which removes only the structurally empty row.
                        ...
                        if (tableRows.Length != rowCount
                            || tableRows.Any(row => !Regex.IsMatch(
                                row.Value,
                                @"<m:oMath(?:\s|>)",
                                RegexOptions.IgnoreCase
                                | RegexOptions.CultureInvariant)))
                            return Fail(
                                $"direct-table-empty-or-unowned-row formulaId={formulaId} rows={rowCount}");
                    }
                ...
                {
                    var tabEvidence = string.Join(
                        "|",
                        Regex.Matches(
                                paragraphXml,
                                @"<w:tabs(?:\s[^>]*)?>.*?</w:tabs>|<w:tab\b[^>]*/>",
                                RegexOptions.IgnoreCase
                                | RegexOptions.CultureInvariant
                                | RegexOptions.Singleline)
                            .Cast<Match>()
                    ...
                    return Fail(
                        $"numbering-paragraph-tab-geometry-invalid formulaId={formulaId} owner=ole "
                        + $"center={hasCenterTabStop} right={hasRightTabStop} tabs={layoutTabCount} "
            ...
            // Visible-owner bookmarks move with an equation's tab paragraph.
            // Its external SEQ caption can remain behind on Cut: keep the result
            // by FormulaId, but never use that caption's position as formula order.
            var ownerPositions = visibleStartMatches.ToDictionary(
                match => Guid.ParseExact(match.Groups["guid"].Value, "N").ToString("D"),
                match => match.Index,
                StringComparer.OrdinalIgnoreCase);
            ...
            var claimedNumberResultTokens = new HashSet<int>();
            foreach (Match startMatch in startMatches)
            {
                ...
                var resultTokens = Regex.Matches(segment,
                    @"<(?:w|m):t(?:\s[^>]*)?>(?<text>.*?)</(?:w|m):t>",
                    RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Singleline);
                if (resultTokens.Count == 0
                    || !claimedNumberResultTokens.Add(startMatch.Index + startMatch.Length
                        + resultTokens[resultTokens.Count - 1].Index))
                    return Fail($"number-bookmarks-share-physical-result formulaId={formulaId}");
                result.Add(new NativeEquationCaptionEntry(
                    formulaId,
                    ownerPositions[formulaId],
                    numberText));
            }
        ...
        IReadOnlyList<NativeEquationCaptionEntry> openXmlEntries)
    {
        ...
        try
        {
            ...
            foreach (var entry in openXmlEntries)
            {
                ...
                try
                {
                    // VTEq_ moves with the visible host in all current layouts;
                    // VTEqNum_ may point to an external caption left behind by Cut.
                    bookmark = bookmarks[EquationBookmarkName(entry.FormulaId)];
                    range = bookmark.Range;
                    result.Add(new NativeEquationCaptionEntry(
                        entry.FormulaId,
                        range.Start,
        ...
        int numberedFormulaCount)
    {
        ...
        {
            content = document.Content;
            var xmlText = WordDocumentXml.Read(document) ?? string.Empty;
        ...
        string formatId)
    {
        ...
        // Acceptance and migration fixtures use this helper to configure the
        // document under test. They must never mutate the real user's default
        // numbering preference in HKCU; otherwise a heading-numbered fixture can
        // leak into the next ordinary document opened by the user.
        if (!string.Equals(
                Environment.GetEnvironmentVariable("VISUALTEX_VSTO_ACCEPTANCE"),
                "1",
                StringComparison.Ordinal))
    ...
    internal static void BuildFormulaNumberingScaffoldForConversion(
        ...
        bool knownFreshConversionTarget = false)
    {
        ...
        ConfigureNumberedDisplayFormula(
            ...
            knownFreshConversionTarget: knownFreshConversionTarget);
    }
    ...
    internal static int BuildNewOmmlNumberingBatch(
        ...
        IReadOnlyDictionary<string, FormulaMetadata>? verifiedFreshMetadata = null,
        WordOmmlNativeSource.LiveInsertionOwners? verifiedFreshInsertions = null,
        bool verifiedFreshBareOmml = false,
        IReadOnlyDictionary<string, string>? verifiedFreshSourceOmml = null,
        bool allowExternalCleanTableTemplate = true)
    {
        var built = 0;
        ...
        try
        {
            if (cleanupCommittedSourceSpacing && allowExternalCleanTableTemplate)
            {
                batchSections = document.Sections;
                allowCleanTableTemplate = batchSections.Count == 1;
            }
        if (verifiedFreshBareOmml
            && (verifiedFreshSourceOmml is null
                || verifiedFreshSourceOmml.Count != formulaIds
                    .Distinct(StringComparer.OrdinalIgnoreCase)
                    .Count()
                || formulaIds.Any(formulaId =>
                    !verifiedFreshSourceOmml.ContainsKey(formulaId))))
            throw new InvalidDataException(
                "Verified fresh OMML numbering requires the exact prepared source inventory.");
        }
        ...
        Range ResolveBatchSourceRange(FormulaMetadata metadata, out bool usedVerifiedOwner)
        {
            usedVerifiedOwner = false;
            Range? candidate;
            if (verifiedFreshInsertions is not null)
            {
                candidate = verifiedFreshInsertions.TryReadVerifiedOwner(metadata.FormulaId)
                    ?? throw new InvalidDataException(
                        $"Verified fresh OMML owner {metadata.FormulaId} disappeared before numbering.");
                usedVerifiedOwner = true;
            }
            else
            {
                candidate = WordOmmlFormulaStore.GetEquationRangeVerifiedForStructuralEdit(
                    document,
                    metadata.FormulaId,
                    metadata);
            }
        ...
        try
        {
            var verifiedInventoryOwners = 0;
            foreach (var formulaId in formulaIds.Distinct(StringComparer.OrdinalIgnoreCase))
            {
                // Format conversion may provide the exact metadata objects that the
                // preceding document-wide fingerprint pass just validated and saved.
                // Other callers keep the established durable-store path.
                FormulaMetadata metadata;
                if (verifiedFreshMetadata is not null
                    && verifiedFreshMetadata.TryGetValue(formulaId, out var verifiedMetadata))
                    metadata = verifiedMetadata;
                else
                    metadata = WordOmmlFormulaStore.TryRead(document, formulaId)
                        ?? throw new InvalidDataException($"The new OMML identity {formulaId} is missing.");
                if (!metadata.Numbered || metadata.DisplayMode != "block")
                    throw new InvalidDataException($"The new OMML identity {formulaId} is not a numbered display formula.");
                Range? range = null;
                ...
                try
                {
                    range = ResolveBatchSourceRange(metadata, out var usedVerifiedOwner);
                    if (usedVerifiedOwner) verifiedInventoryOwners++;
            ...
            TraceStage(verifiedInventoryOwners == entries.Count && entries.Count > 0
                ? "inventory-verified-owner"
                : "inventory");
            ...
            foreach (var entry in entries.OrderBy(item => item.Position))
            {
                ...
                try
                {
                    ...
                    range = ResolveBatchSourceRange(metadata, out var usedVerifiedOwner);
                    TraceStage(usedVerifiedOwner
                        ? "locate-target-verified-owner"
                        : "locate-target");
                    ...
                    ConfigureNumberedNativeOmmlDisplay(
                        ...
                        preparedFreshBareOmml:
                            verifiedFreshBareOmml
                                && verifiedFreshSourceOmml!.TryGetValue(
                                    entry.FormulaId,
                                    out var preparedFreshBareOmml)
                                    ? preparedFreshBareOmml
                                    : null);
                    TraceStage("true-display-host");
                    if (usedVerifiedOwner)
                        verifiedFreshInsertions!.RebindAfterVerifiedNumbering(entry.FormulaId, range);

                    if (committedTableStarts is not null)
                    {
                        // A numbered OMML formula inside a user table intentionally
                        // remains in that cell and uses the native #(SEQ) wrapper.
                        // Do not mistake the outer user table for the 1x3 table that
                        // this cleanup list is designed to compact/delete around.
                        ...
                        {
                            TraceStage("capture-table-cell-hash-host");
                        }
                        else
                        {
                            ...
                            try
                            {
                                ...
                                if (!IsManagedNumberedEquationTable(
                                        ...
                                        entry.FormulaId))
                                    throw new InvalidDataException(
                                        $"The newly numbered OMML {entry.FormulaId} resolved to an unrelated Word table.");
        ...
        string? plannedNumberPrefix = null)
    {
        ...
        if (metadata.Numbered)
        {
            ...
            var rebuiltWithPreservedNumberPlan =
                !hadCompleteOwnedArtifacts
                && !numberingOrderMayHaveChanged
                && plannedNumberOrdinal.HasValue
                && plannedNumberPrefix is not null;
            ...
            var visibleNumberCreated = ConfigureNumberedDisplayFormula(
                ...
                reuseExistingScaffold:
                    knownNumberedTable is not null
                    || (hadCompleteOwnedArtifacts && !ContainsNativeOmml(formulaRange))
            ...
            if (rebuiltWithPreservedNumberPlan)
            {
                // OLE -> OMML conversion replaces the physical host but not the
                // formula's document position or logical sequence slot. The caller
                // captured the old SEQ ordinal/prefix before removing its aliases,
                // and ConfigureNumberedDisplayFormula used that exact plan for the
                // new direct-SEQ cell. No suffix sequence or body REF changed, so a
                // document-wide caption/field inventory here is pure overhead.
                TraceStage("preserved-number-plan");
            }
            else if (rebuiltStableNumberingSlot)
            {
            ...
            {
                var appendedSequenceUpdated = TryUpdateAppendedNativeEquationSequenceField(
                    document,
                    metadata.FormulaId,
                    out var appendedNumberChanged);
                if (appendedSequenceUpdated)
                {
                ...
                {
                    // Two individual suffix field passes are acceptable for a tiny
                    // document, but become quadratic once dozens of numbered
                    // formulas are present. Large documents use the validated
                    // single-batch suffix path below first; the range path remains
                    // a compatibility fallback for small/legacy hosts.
                    var continuousRangeUpdated =
                        !ShouldPreferScopedSequenceSuffixUpdate(document)
                        && TryUpdateInsertedContinuousEquationFieldRanges(
                            document,
                            metadata.FormulaId);
                    if (continuousRangeUpdated)
                    {
                        TraceStage("continuous-range-fast");
                    }
                    else
                    {
                        var continuousSuffixUpdated = TryUpdateInsertedEquationSequenceSuffixWithinScope(
                            document,
                            metadata.FormulaId,
                            out var changedFormulaNumbers,
                            out var referencesAlreadyUpdatedFrom);
                        if (continuousSuffixUpdated)
                        {
                        ...
                        else
                        {
                            ...
                            IReadOnlyList<NativeEquationCaptionEntry> fallbackOpenXmlCaptions =
                                Array.Empty<NativeEquationCaptionEntry>();
                            IReadOnlyDictionary<string, int> knownFallbackReferenceCounts =
                                new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);

                            // Word 2021 terminates an active Custom UndoRecord when a
                            // main-story Range.WordOpenXML snapshot is requested. A
                            // create/edit transaction must therefore prove numbering
                            // ownership from the already-created bookmarks and fields
                            // instead of invoking the document-wide XML health pass.
                            // Outside a custom transaction, retain the mature OpenXML
                            // validation and its faster reference-count inventory.
                            var customUndoActive = IsCustomUndoRecordActive(document);
                            var healthyOpenXmlRead = !customUndoActive
                                && TryReadHealthyEquationNumberArtifactsFromOpenXml(
                                    document,
                                    out fallbackOpenXmlCaptions,
                                    out knownFallbackReferenceCounts);
                            if (customUndoActive)
                            {
                                var nativeSequenceName = GetNativeEquationSequenceName(document);
                                if (!TryReadCompleteEquationNumberArtifactsFromBookmarks(
                                        document,
                                        nativeSequenceName,
                                        out var transactionCaptions)
                                    || !transactionCaptions.Any(caption => string.Equals(
                                        caption.FormulaId,
                                        metadata.FormulaId,
                                        StringComparison.OrdinalIgnoreCase)))
                                    throw new InvalidDataException(
                                        "VisualTeX could not prove a complete equation-number bookmark scaffold without ending Word's undo transaction.");

                                var transactionFormat = ReadEquationNumberFormat(document);
                                fallbackChangedFormulaNumbers =
                                    UpdateNativeEquationSequenceFieldsIncremental(
                                        ...
                                        transactionCaptions,
                                        transactionFormat,
                                        trustedHealthyDirectTables: false);
                                TraceStage("sequence-fallback-custom-undo");
                            }
                            else if (healthyOpenXmlRead)
                            {
        ...
        {
            // A VisualTeX display OLE uses the same center/right tab paragraph as
            // a numbered display formula; the unnumbered form simply omits the
            // trailing tab/number artifacts. Keeping one paragraph-layout model
            // prevents Word from giving the paragraph mark a different line-box
            // placement when numbering is toggled.
            var containsNativeOmml = ContainsNativeOmml(formulaRange);
            ConfigureEquationParagraph(formulaRange, numbered: false);
            if (!containsNativeOmml)
            {
                EnsureLeadingEquationTab(document, formulaRange);
                AlignDisplayOleObjectToParagraphBaseline(
                    document,
                    formulaRange,
                    formulaHeightPoints,
                    metadata);
    ...
    internal static Range? FindVisibleEquationNumberRange(
        ...
        string formulaId)
    {
        // New VisualTeX body numbering is self-contained in the same paragraph
        // as the OLE and intentionally owns no always-on visible-number bookmark.
        // Resolve that field tree first; the legacy bookmark paths below remain
        // only for older documents and OMML migration compatibility.
        var selfContained =
        ...
        try
        {
            ...
            if (bookmarks.Exists(name))
            {
                ...
                // Word 2021 can expand a bookmark ending at the paragraph boundary
                // across the paragraph terminator. Around fields, Range.Text and
                // story positions are not one-to-one, so trimming End by character
                // count is unsafe. Canonicalize against the first owning paragraph's
                // exact editable-body boundary instead. A visible equation-number
                // label is never allowed to own a paragraph/cell terminator.
                Paragraphs? visibleParagraphs = null;
                Paragraph? visibleParagraph = null;
                Range? visibleParagraphRange = null;
                Range? trailing = null;
                try
                {
                    visibleParagraphs = range.Paragraphs;
                    if (visibleParagraphs.Count > 0)
                    {
                        visibleParagraph = visibleParagraphs[1];
                        visibleParagraphRange = visibleParagraph.Range.Duplicate;
                        var terminatorLength =
                            GetWordParagraphStructuralTerminatorLength(
                                visibleParagraphRange);
                        var editableEnd = Math.Max(
                            visibleParagraphRange.Start,
                            visibleParagraphRange.End - terminatorLength);
                        if (range.Start < editableEnd
                            && range.End > editableEnd)
                            range.End = editableEnd;
                    }

                    // Defensive normalization for unusual Word stories whose
                    // paragraph owner does not expose a structural terminator.
                    while (range.End > range.Start)
                    {
                        Release(trailing);
                        trailing = document.Range(range.End - 1, range.End);
                        var trailingText = trailing.Text ?? string.Empty;
                        if (!string.Equals(
                                trailingText,
                                "\r",
                                StringComparison.Ordinal)
    ...
    internal static Range? FindNumberingOwnerRange(
        ...
        string formulaId)
    {
        var selfContainedOwner =
            WordVisualTeXParagraphNumbering
                .FindOwnerParagraphRange(
                    document,
                    formulaId);
        if (selfContainedOwner is not null)
            return selfContainedOwner;
        ...
        try
        {
            ...
            var nativeHashSequenceHost =
                HasManagedNativeOmmlHashSequenceHost(document, formulaId);
            ...
            try
            {
                if (!nativeHashSequenceHost
                    && RangeIsWhollyWithinTable(visibleRange))
                {
                    // A user cell in column three can own a normal OLE + REF
                    // paragraph. Only a proven managed number-only third cell
                    // may widen its owner to the whole legacy equation row.
                    table = FindNumberedEquationTable(document, formulaId);
                    if (table is not null)
                    {
                        columns = table.Columns;
                        if (columns.Count == 3
                            && TryGetManagedNumberTableRowIndex(table, visibleRange, 3, out var rowIndex))
                        {
                            Row? row = null;
                            try
                            {
                                row = table.Rows[rowIndex];
                                ownerRange = row.Range.Duplicate;
                            }
                            finally { Release(row); }
        ...
        catch (Exception error)
        {
            ...
            {
                TraceNumberingPerformance(
                    $"[perf] numbering-owner-range-failed formulaId={formulaId} "
        ...
        string formulaId)
    {
        ...
        InlineShapes? centerShapes = null;
        try
        {
            bookmarks = document.Bookmarks;
            var name = EquationBookmarkName(formulaId);
            ...
            centerShapes = centerCellRange.InlineShapes;
            if (centerMaths.Count + centerShapes.Count != 1)
                return null;
    ...
    internal static Table? FindNumberedEquationTableByFormulaOwner(
        Document document,
        Range formulaRange,
        string formulaId)
    {
        if (document is null || formulaRange is null
            || string.IsNullOrWhiteSpace(formulaId))
            return null;

        // The visible VTEq_* bookmark is the normal and cheapest ownership proof.
        // Word can nevertheless expand that bookmark into column 2 after an
        // equation object is replaced or a neighbouring invisible typing anchor is
        // removed. In that damaged-document state the generated 1x3 row is still
        // unambiguous: the formula's own durable identity owns the sole payload in
        // column 2 and column 3 owns exactly one VisualTeX SEQ field. Recover that
        // row so callers can remove it instead of leaving an empty table behind.
        var bookmarked = FindNumberedEquationTable(document, formulaId);
        if (bookmarked is not null) return bookmarked;
        ...
        InlineShapes? formulaShapes = null;
        InlineShape? formulaShape = null;
        ...
        Bookmarks? bookmarks = null;
        Bookmark? driftedVisibleBookmark = null;
        Range? driftedVisibleRange = null;
        Table? Fail(string reason)
        {
            TraceNumberingPerformance(
                $"[recovery] numbered-table-owner-rejected formulaId={formulaId} reason={reason}");
            return null;
        }
        try
        {
            tables = formulaRange.Tables;
            if (tables.Count != 1) return Fail($"formula-tables={tables.Count}");
            table = tables[1];
            rows = table.Rows;
            columns = table.Columns;
            if (columns.Count != 3
                || !TryGetManagedNumberTableRowIndex(
                    ...
                    out var rowIndex))
                return Fail($"table-columns={columns.Count}-or-formula-row");

            leftCell = table.Cell(rowIndex, 1);
            formulaCell = table.Cell(rowIndex, 2);
            numberCell = table.Cell(rowIndex, 3);
            ...
            if (!ContainsOnlyStructuralWordText(leftRange.Text))
                return Fail("left-cell-user-text");

            formulaMaths = formulaCellRange.OMaths;
            formulaShapes = formulaCellRange.InlineShapes;
            if (formulaMaths.Count + formulaShapes.Count != 1)
                return Fail(
                    $"center-payload-maths={formulaMaths.Count}-shapes={formulaShapes.Count}");
            if (formulaShapes.Count == 1)
            {
                formulaShape = formulaShapes[1];
                var metadata = WordFormulaMetadataReader.IsNativeOle(formulaShape)
                    ? WordFormulaMetadataReader.TryReadEmbeddedNativeOle(formulaShape)
                    : null;
                if (metadata is null
                    || !string.Equals(
                        metadata.FormulaId,
                        ...
                        StringComparison.OrdinalIgnoreCase))
                    return Fail(
                        $"ole-identity={(metadata?.FormulaId ?? "missing")}");
            }
            else
            {
                ommlBookmark = WordOmmlFormulaStore.FindAtRange(document, formulaRange);
                var metadata = ommlBookmark is null
                    ? null
                    : WordOmmlFormulaStore.TryRead(document, ommlBookmark);
                if (metadata is null
                    || !string.Equals(
                        metadata.FormulaId,
                        formulaId,
                        StringComparison.OrdinalIgnoreCase))
                    return Fail(
                        $"omml-identity={(metadata?.FormulaId ?? "missing")}");
            }

            numberMaths = numberCellRange.OMaths;
            numberShapes = numberCellRange.InlineShapes;
            numberFields = numberCellRange.Fields;
            if (numberMaths.Count != 0
                || numberShapes.Count != 0)
                return Fail(
                    $"number-payload-maths={numberMaths.Count}-shapes={numberShapes.Count}-fields={numberFields.Count}");
            if (numberFields.Count == 1)
            {
                ...
                if (!IsVisualTeXSequenceFieldCode(numberCode.Text))
                    return Fail($"number-field-code={numberCode.Text}");
                var visibleNumberText = NormalizeNativeEquationNumberText(
                    numberCellRange.Text);
                if (!visibleNumberText.StartsWith("(", StringComparison.Ordinal)
                    || !visibleNumberText.EndsWith(")", StringComparison.Ordinal))
                    return Fail($"number-text={visibleNumberText}");
            }
            else if (numberFields.Count == 0)
            {
                // The same Word mutation that expands VTEq_* into the formula cell
                // can consume the generated right-cell field first. Recover only
                // the exact one-row scaffold whose left/right cells are otherwise
                // empty and whose formula-specific visible bookmark now owns the
                // same column-2 row. This conjunction cannot match an ordinary
                // user table merely because it contains an equation.
                if (rows.Count != 1
                    || !ContainsOnlyStructuralWordText(numberCellRange.Text))
                    return Fail(
                        $"number-field-missing-rows={rows.Count}-text={NormalizeNativeEquationNumberText(numberCellRange.Text)}");
                bookmarks = document.Bookmarks;
                var visibleNumberName = EquationBookmarkName(formulaId);
                if (!bookmarks.Exists(visibleNumberName))
                    return Fail("number-field-and-visible-bookmark-missing");
                driftedVisibleBookmark = bookmarks[visibleNumberName];
                driftedVisibleRange = driftedVisibleBookmark.Range.Duplicate;
                if (!TryGetManagedNumberTableRowIndex(
                        table,
                        driftedVisibleRange,
                        expectedColumnIndex: 2,
                        out var driftedRowIndex)
                    || driftedRowIndex != rowIndex)
                    return Fail("missing-field-visible-bookmark-not-in-formula-cell");
            }
            else
            {
                return Fail($"number-fields={numberFields.Count}");
            }

            TraceNumberingPerformance(
                $"[recovery] numbered-table-owned-by-formula formulaId={formulaId} row={rowIndex} reason=visible-bookmark-drift");
            var result = table;
            table = null;
            return result;
        }
        ...
        {
            return Fail("com-error");
        }
        finally
        {
            Release(driftedVisibleRange);
            Release(driftedVisibleBookmark);
            Release(bookmarks);
        ...
        try
        {
            ...
            if (formulaTables.Count == 0 || numberTables.Count == 0)
                return (formulaRange.Start <= numberRange.Start
                    ...
                    || (formulaTables.Count == 0 && numberTables.Count == 0
                        && OwnsLocalOleNumberReference(formulaRange, numberRange, formulaId));
    ...
    private static bool OwnsLocalOleNumberReference(Range formulaRange, Range numberRange, string formulaId)
    {
        Paragraphs? paragraphs = null;
        Paragraph? paragraph = null;
        Range? host = null;
        ...
        try
        {
            // In the current tab layout the visible REF is beside the OLE, not
            // inside its Range. Requiring containment misclassified every healthy
            // tab host as a new insertion, causing full-document numbering on Edit.
            if (formulaRange.StoryType != numberRange.StoryType) return false;
            paragraphs = formulaRange.Paragraphs;
            if (paragraphs.Count != 1) return false;
            paragraph = paragraphs[1]; host = paragraph.Range;
            if (numberRange.Start < formulaRange.End || numberRange.End > host.End) return false;
            shapes = host.InlineShapes;
            if (shapes.Count != 1) return false;
            shape = shapes[1]; actual = shape.Range;
            if (actual.Start != formulaRange.Start || actual.End != formulaRange.End
                || !WordFormulaMetadataReader.IsNativeOle(shape)) return false;
            fields = host.Fields;
            for (var i = 1; i <= fields.Count; i++)
            {
                ...
                try
                {
                    ...
                    code = field.Code;
                    if (!IsReferenceToBookmark(code.Text, NativeNumberBookmarkName(formulaId))) continue;
                    result = field.Result;
                    if (result.Start >= numberRange.Start && result.End <= numberRange.End) return true;
                }
        ...
        finally { Release(fields); Release(actual); Release(shape); Release(shapes); Release(host); Release(paragraph); Release(paragraphs); }
    }
        ...
        try
        {
            ...
            for (var index = 1; index <= fields.Count; index++)
            {
                ...
                try
                {
                    ...
                    WordEquationReferenceFields.RefreshFieldPreservingFormatting(field);
    ...
    internal static int RepairLeakedNativeCaptionFrames(Document document)
    {
        ...
        foreach (var captionName in captionNames)
        {
            ...
            try
            {
                ...
                // InsertParagraphBefore on a collapsed range in the middle of a
                // paragraph inserts a paragraph before the *owner paragraph* on
                // some Office 2019/2021 builds; it does not reliably split at the
                // collapsed coordinate. That leaves the leaked body text inside
                // VTEqCap or moves it ahead of the caption. Insert the paragraph
                // mark as exact story text at VTEqNum.End so the SEQ remains in
                // the first paragraph and every original trailing character moves
                // intact into the new visible body paragraph.
                splitRange.Text = "\r";
    ...
    internal static WordCharacterFormatting CaptureNumberedBodyCharacterFormatting(Document document, string formulaId)
    {
        Range? number = null;
        try
        {
            // The number host's paragraph mark retains the source body format.
            // Neither OMath/OLE typography nor the clipped SEQ caption is a body source.
            number = FindVisibleEquationNumberTextRange(document, formulaId)
                ?? throw new InvalidDataException("The numbered formula has no visible number host for body continuation.");
            return WordCharacterFormatting.CaptureParagraphMark(number);
        }
        finally { Release(number); }
    }

    private static bool TryTrimExpandedNativeCaptionBookmarksAfterStyle(
        Document document,
        string formulaId,
        Range captionParagraphRange)
    {
        Bookmarks? bookmarks = null;
        Bookmark? captionBookmark = null;
        Bookmark? numberBookmark = null;
        Range? captionBookmarkRange = null;
        Range? numberBookmarkRange = null;
        Range? trailing = null;
        Range? canonicalCaption = null;
        Range? canonicalNumber = null;
        OMaths? maths = null;
        InlineShapes? shapes = null;
        Fields? fields = null;
        Tables? tables = null;
        ...
        try
        {
            ...
            captionBookmark = bookmarks[captionName];
            numberBookmark = bookmarks[numberName];
            captionBookmarkRange = captionBookmark.Range;
            numberBookmarkRange = numberBookmark.Range;
            var overflowEnd = Math.Max(captionBookmarkRange.End, numberBookmarkRange.End);
            if (overflowEnd <= captionParagraphRange.End)
                return false;

            trailing = document.Range(captionParagraphRange.End, overflowEnd);
            var trailingText = trailing.Text ?? string.Empty;
            if (!trailingText.All(character =>
                    char.IsWhiteSpace(character) || character == '\a'))
                return false;
            maths = trailing.OMaths;
            shapes = trailing.InlineShapes;
            fields = trailing.Fields;
            tables = trailing.Tables;
            controls = trailing.ContentControls;
            frames = trailing.Frames;
            if (maths.Count != 0
                || shapes.Count != 0
                || fields.Count != 0
                || tables.Count != 0
            ...
            var canonicalNumberStart = Math.Max(
                captionParagraphRange.Start,
                numberBookmarkRange.Start);
            var canonicalNumberEnd = Math.Min(
                captionParagraphRange.End,
                numberBookmarkRange.End);
            if (canonicalNumberEnd <= canonicalNumberStart)
                throw new InvalidDataException(
                    "The native equation number bookmark no longer intersects its caption paragraph.");
            ...
            canonicalCaption = document.Range(
                captionBookmarkRange.Start,
                captionParagraphRange.End);
            canonicalNumber = document.Range(
                canonicalNumberStart,
                canonicalNumberEnd);
            captionBookmark = bookmarks.Add(captionName, canonicalCaption);
            numberBookmark = bookmarks.Add(numberName, canonicalNumber);

            Release(captionBookmarkRange);
            captionBookmarkRange = captionBookmark.Range;
            Release(numberBookmarkRange);
            numberBookmarkRange = numberBookmark.Range;
            if (captionBookmarkRange.End > captionParagraphRange.End
                || numberBookmarkRange.End > captionParagraphRange.End)
                throw new InvalidDataException(
                    "Word re-expanded the native equation caption bookmarks after their final binding.");
        ...
        finally
        {
            ...
            Release(shapes);
            Release(maths);
            Release(canonicalNumber);
            Release(canonicalCaption);
            Release(trailing);
            Release(numberBookmarkRange);
            Release(captionBookmarkRange);
            Release(numberBookmark);
    ...
    private static bool TryNormalizeExpandedNativeCaptionBeforeTyping(
        Document document,
        string formulaId,
        Range captionRange,
        Range captionParagraphRange)
    {
        if (captionRange.End <= captionParagraphRange.End)
            return false;
        ...
        Bookmark? captionBookmark = null;
        Range? canonicalCaption = null;
        Range? refreshedCaption = null;
        Range? numberRange = null;
        try
        {
            trailing = document.Range(captionParagraphRange.End, captionRange.End);
            var trailingText = trailing.Text ?? string.Empty;
            if (!trailingText.All(character =>
                    char.IsWhiteSpace(character) || character == '\a'))
                return false;
            ...
            // Compatibility-mode Word can expand the hidden VTEqCap_<id>
            // bookmark and its clipping Frame across the pre-existing blank
            // paragraph at an insertion point before EnsureNormalTypingParagraph...
            // gets a chance to create/reuse the continuation. Canonicalize that
            // ownership first: the hidden caption is exactly its first paragraph;
            // the following structurally-empty paragraph belongs to body typing.
            frames = captionRange.Frames;
            if (frames.Count > 0)
            {
                frame = frames[1];
            ...
            if (!TryGetNativeCaptionRanges(
                    ...
                    formulaId,
                    GetNativeEquationSequenceName(document),
                    out refreshedCaption,
                    out numberRange)
                || refreshedCaption is null
                || numberRange is null)
                throw new InvalidDataException(
                    "VisualTeX could not rebind an expanded native equation caption to its own paragraph.");

            StyleNativeCaption(
                refreshedCaption,
                numberRange,
                cleanupLegacyFrames: false);
            TryTrimExpandedNativeCaptionBookmarksAfterStyle(
                document,
                formulaId,
                captionParagraphRange);
            return true;
        }
        finally
        {
            Release(numberRange);
            Release(refreshedCaption);
            Release(canonicalCaption);
            Release(captionBookmark);
            Release(bookmarks);
            Release(frame);
        ...
        Table? knownDirectTable = null)
    {
        if (knownDirectTable is not null
            && IsManagedNativeOmmlDirectTable(knownDirectTable))
        {
            Cell? knownNumberCell = null;
            Range? knownNumberRange = null;
            try
            {
                knownNumberCell = knownDirectTable.Cell(1, 3);
                knownNumberRange = knownNumberCell.Range.Duplicate;
                var knownBodyFormatting =
                    WordCharacterFormatting.CaptureParagraphMark(knownNumberRange);
            ...
            {
                Release(knownNumberRange);
                Release(knownNumberCell);
            }
        }

        var bodyFormatting = CaptureNumberedBodyCharacterFormatting(document, formulaId);
        ...
        Range? trimmedCaptionRange = null;
        ...
        try
        {
            ...
            if (TryNormalizeExpandedNativeCaptionBeforeTyping(
                    document,
                    formulaId,
                    captionRange,
                    captionParagraphRange))
            {
                ...
                if (!bookmarks.Exists(captionName))
                    throw new InvalidDataException(
                        "VisualTeX lost the normalized native equation caption bookmark.");
                captionBookmark = bookmarks[captionName];
                captionRange = captionBookmark.Range;
                captionParagraphs = captionRange.Paragraphs;
                captionParagraph = captionParagraphs[1];
            ...
            if (nextParagraphStart < content.End)
            {
                ...
                if (!inTable && typingFrames.Count == 0)
                {
                    ...
                    if (typingParagraphs.Count > 0)
                    {
                        ...
                        {
                            // Whole-paragraph Word paste can expand VTEqCap_<id>
                            // across the newly pasted empty continuation paragraph.
                            // The caption itself is still the first hidden SEQ
                            // paragraph; the trailing paragraph is ordinary body
                            // space and must not become caption-owned. Trim only
                            // this exact one-paragraph overflow before validating
                            // the continuation so formula+number and full-paragraph
                            // paste share the same stable typing behavior.
                            if (captionRange.End > captionParagraphRange.End
                                && typingRange.Start == captionParagraphRange.End
                                && typingRange.End <= captionRange.End)
                            {
                                trimmedCaptionRange = document.Range(
                                    captionRange.Start,
                                    captionParagraphRange.End);
                                captionBookmark.Delete();
                                Release(captionBookmark);
                            ...
                            return NormalizeBodyTypingParagraph(document, typingRange, bodyFormatting);
                        }
                    }
                }
            ...
            captionParagraph = captionParagraphs[1];
            captionParagraphRange = captionParagraph.Range;
            // Compatibility-mode Word can expand both VTEqCap_<id> and
            // VTEqNum_<id> when the clipping Frame is rebuilt. Do not restyle the
            // caption again here: that would repeat the same expansion. Finalize
            // both bookmark extents only after all paragraph/Frame mutations are
            // complete, then re-resolve every live RCW before using the body tail.
            if (TryTrimExpandedNativeCaptionBookmarksAfterStyle(
                    document,
                    formulaId,
                    captionParagraphRange))
            {
                ...
                if (!bookmarks.Exists(captionName))
                    throw new InvalidDataException(
                        "VisualTeX lost the finalized native equation caption bookmark after rebuilding its frame.");
                captionBookmark = bookmarks[captionName];
                captionRange = captionBookmark.Range;
                captionParagraphs = captionRange.Paragraphs;
                captionParagraph = captionParagraphs[1];
        ...
        Document document)
    {
        ...
        try
        {
            ...
            if (captionMatches.Count > 0 && Guid.TryParseExact(
    ...
    internal static void RemoveFormulaNumberingArtifacts(
        ...
        Range? replacingCompleteHost = null,
        bool deferCaptionFlowTailCleanup = false,
        bool completeHostAlreadyVerified = false)
    {
        Bookmark? formulaBookmark = null;
        Range? formulaRange = null;
        try
        {
            ...
            {
                formulaRange = WordOmmlFormulaStore.GetEquationRange(formulaBookmark);
                if (replacingCompleteHost is not null)
                {
                    // A conversion replacing a complete managed row owns its
                    // formula and ordinary number together. The native row
                    // replacement removes both atomically; legacy bookmark-text
                    // deletion can touch a cell terminator or a retargeted range.
                    Tables? tables = null;
                    Table? table = null;
                    Range? tableRange = null;
                    try
                    {
                        if (!completeHostAlreadyVerified
                            && !HasReusableNumberedNativeOmmlDirectTableHost(
                                document,
                                formulaRange,
                                formulaId))
                            throw new InvalidDataException("Complete-host replacement requires a verified numbered OMML row.");
                        tables = formulaRange.Tables;
                        table = tables[1];
                        tableRange = table.Range;
                        if (table.Rows.Count != 1 || table.Columns.Count != 3
                            || replacingCompleteHost.StoryType != tableRange.StoryType
                            || replacingCompleteHost.Start != tableRange.Start || replacingCompleteHost.End != tableRange.End)
                            throw new InvalidDataException("The replacement scope is not exactly the formula's isolated managed row.");
                        return;
                    }
                    finally { Release(tableRange); Release(table); Release(tables); }
                }
        ...
        if (replacingCompleteHost is not null)
            throw new InvalidDataException("The complete replacement host has no formula identity.");
        ...
        RemoveNativeCaption(
            ...
            deferFlowTailCleanup: deferCaptionFlowTailCleanup);
    }
        ...
        bool knownFreshConversionTarget = false)
    {
        ...
        try
        {
            ...
            if (IsManagedNumberedEquationTable(document, formulaRange, formulaId))
            {
            ...
            if (IsManagedNumberedEquationTable(document, activeFormulaRange, formulaId))
                throw new InvalidOperationException(
    ...
    private static bool IsManagedNumberedEquationTable(
        Document document,
        Range formulaRange,
        string formulaId)
    {
        if (document is null
            || formulaRange is null
            || string.IsNullOrWhiteSpace(formulaId))
            return false;
        ...
        try
        {
            ...
            columns = table.Columns;
            if (columns.Count != 3
                || tableRange.StoryType != formulaRange.StoryType
                || formulaRange.Start < tableRange.Start
                || formulaRange.End > tableRange.End
                || !TryGetManagedNumberTableRowIndex(
                    table,
                    formulaRange,
                    expectedColumnIndex: 2,
            ...
            numberBookmark = bookmarks[numberName];
            numberRange = numberBookmark.Range;
            return TryGetManagedNumberTableRowIndex(
                       table,
                       numberRange,
                       expectedColumnIndex: 3,
    ...
    {
        // Column count is not ownership. In particular, the third cell of an
        // ordinary three-column user table may contain another formula, and must
        // never become our number slot. Require the equation's actual identity
        // and its matching same-row third-column number bookmark.
        Document? document = null;
        InlineShapes? shapes = null;
        InlineShape? shape = null;
        Bookmark? bookmark = null;
        try
        {
            ...
            {
                bookmark = WordOmmlFormulaStore.FindAtRange(document, formulaRange);
                if (bookmark is not null)
                    metadata = WordOmmlFormulaStore.TryRead(document, bookmark);
            }
            return metadata?.Numbered == true
                && IsManagedNumberedEquationTable(document, formulaRange, metadata.FormulaId);
        }
        catch { return false; }
        finally { Release(bookmark); Release(shape); Release(shapes); Release(document); }
    }

    internal static int GetManagedNumberTableRowIndex(
        Table table, Range ownedRange, int expectedColumnIndex)
    {
        if (!TryGetManagedNumberTableRowIndex(table, ownedRange, expectedColumnIndex, out var rowIndex))
            throw new InvalidDataException(
                "The formula or number range is not owned by exactly one managed table row.");
        return rowIndex;
    }
        ...
        out int rowIndex)
    {
        ...
        {
            TraceNumberingPerformance($"[perf] managed-row-ownership-rejected check={ownershipCheck} reason={reason}");
            return false;
        }
        try
        {
            ...
            if (cells.Count != 1) return RejectOwnership($"cells={cells.Count} range={ownedRange.Start}:{ownedRange.End}");
            cell = cells[1];
            ...
            if (ownedRange.StoryType != cellRange.StoryType
                || ownedRange.Start < cellRange.Start
                || ownedRange.End > cellRange.End)
                return RejectOwnership($"range={ownedRange.StoryType}/{ownedRange.Start}:{ownedRange.End} cell={cellRange.StoryType}/{cellRange.Start}:{cellRange.End}");
            ownershipCheck = "cell-table";
            cellTables = cellRange.Tables;
            if (cellTables.Count != 1) return RejectOwnership($"tables={cellTables.Count} cell={cellRange.Start}:{cellRange.End}");
            ...
            if (tableRange.Start != cellTableRange.Start
                ...
                return RejectOwnership($"table={tableRange.Start}:{tableRange.End} cellTable={cellTableRange.Start}:{cellTableRange.End}");
        ...
        {
            rowIndex = 0;
            return RejectOwnership(error.ToString());
        }
        finally
        {
    ...
    internal static Table IsolateManagedNumberTableRow(Table table, Range formulaRange)
    {
        ...
        try
        {
            ...
            {
                object firstTrailingRow = rowIndex + 1;
                trailing = table.Split(ref firstTrailingRow);
            }
            if (rowIndex > 1)
            {
                object firstOwnedRow = rowIndex;
                return table.Split(ref firstOwnedRow);
            }
            // Return an independently owned RCW reference, including for row 1.
            Tables? ownedTables = null;
            try
            {
                ownedTables = formulaRange.Tables;
                return ownedTables[1];
            }
            finally { Release(ownedTables); }
        }
        finally
        {
            Release(trailing);
        ...
        string? preparedStandaloneOmml = null)
    {
        ...
        try
        {
            ...
            if (rows.Count > 1)
            {
                // Unnumbering changes the selected row into a paragraph. Isolate
                // that proven owner with Word's native table split; other rows,
                // their formulas, fields and reference targets remain live.
                if (!HasReusableNumberedNativeOmmlDirectTableHost(document, formulaRange, formulaId))
                    return Fail("multirow-host-ownership-not-proven");
                var isolated = IsolateManagedNumberTableRow(table, formulaRange);
                Release(columns);
                Release(rows);
                Release(table);
                table = isolated;
    ...
    private static void ConfigureUnnumberedDisplayFormula(
        ...
        float? formulaHeightPoints = null,
        string? preparedStandaloneOmml = null,
        bool deferMetadataPersistence = false)
    {
        ...
        try
        {
            ...
            ConfigureEquationParagraph(formulaRange, numbered: false);
            EnsureLeadingEquationTab(document, formulaRange);
            if (knownMetadata is not null && formulaHeightPoints is > 0f)
            {
                AlignDisplayOleObjectToParagraphBaseline(
                    document,
                    formulaRange,
                    formulaHeightPoints.Value,
                    knownMetadata);
            }
            else
            {
                // Metadata-free legacy cleanup cannot infer the formula's exported
                // baseline safely. Retain the old neutral fallback for that narrow
                // repair path; normal create/edit/reconcile always supplies both.
    ...
    private static void AlignDisplayOleObjectToParagraphBaseline(
        Document document,
        Range formulaRange,
        ...
        FormulaMetadata metadata)
    {
        ...
        try
        {
            ...
            float? exportedBaseline = metadata.Baseline.HasValue
                ...
                : null;
            var position = 0;
            var usedDisplayInkCenter = false;
            if (!metadata.Numbered)
            {
                float? inkHeightRatio = metadata.WordDisplayPreviewInkHeightRatio.HasValue
                ...
                if (!inkHeightRatio.HasValue || !bottomWhitespaceRatio.HasValue)
                {
                    try
                    {
                        shapes = formulaRange.InlineShapes;
                        if (shapes.Count == 1)
                        {
                            shape = shapes[1];
                            shapeRange = shape.Range;
                            var emfBytes = shapeRange.EnhMetaFileBits as byte[];
                            var metrics = emfBytes is null
                                ? null
                                : OfficeOlePreview.TryMeasureVerticalInkMetrics(emfBytes);
                            if (metrics.HasValue)
                            {
                                inkHeightRatio = metrics.Value.InkHeightRatio;
                                bottomWhitespaceRatio =
                                    metrics.Value.BottomWhitespaceRatio;
                                metadata.WordDisplayPreviewInkHeightRatio =
                                    inkHeightRatio.Value;
                                metadata.WordDisplayPreviewBottomWhitespaceRatio =
                                    bottomWhitespaceRatio.Value;
                            }
                        }
                    }
                    catch
                    {
                        inkHeightRatio = null;
                        bottomWhitespaceRatio = null;
                    }
                }

                var paragraphMarkFontSize = 0f;
                if (inkHeightRatio.HasValue && bottomWhitespaceRatio.HasValue)
                {
                    try
                    {
                        ...
                        if (paragraphs.Count == 1)
                        {
                            ...
                            if (paragraphRange.End > paragraphRange.Start)
                            {
                                        ...
                                        StringComparison.Ordinal))
                                {
                                    ...
                                    paragraphMarkFontSize = paragraphMarkFont.Size;
                                }
                            }
                        }
                    }
                    catch { paragraphMarkFontSize = 0f; }
                }

                if (paragraphMarkFontSize > 0
                    && paragraphMarkFontSize < 256
                    && inkHeightRatio.HasValue
                    && bottomWhitespaceRatio.HasValue)
                {
                    position = WordInlineAlignment.CalculateDisplayInkCenterPosition(
                        actualHeightPoints,
                        inkHeightRatio.Value,
                        bottomWhitespaceRatio.Value,
                        paragraphMarkFontSize);
                    usedDisplayInkCenter = true;
                }
            }

            if (!usedDisplayInkCenter)
            {
                position = WordInlineAlignment.CalculateFontPositionWithLegacyFallback(
                    actualHeightPoints,
                    exportedHeight,
                    exportedBaseline,
            ...
            // A Word inline OLE is a wdFieldEmbed whose Result owns the painted
            // U+0001 object character. Resolve that one local field directly on
            // the normal path instead of creating a COM Range for every character
            // in the complete EMBED field span. The legacy character walk remains
            // the compatibility fallback when Word exposes an unusual field tree.
            if (TrySetEmbeddedOleResultPosition(formulaRange, clamped))
                return;
    ...
    private static bool TrySetEmbeddedOleResultPosition(
        Range formulaRange,
        int position)
    {
        Fields? fields = null;
        ...
        try
        {
            fields = WordFormulaHost.GetLocalFields(formulaRange);
            var matched = false;
            for (var index = 1; index <= fields.Count; index++)
            {
                Release(result);
                ...
                if (!string.Equals(text, "\u0001", StringComparison.Ordinal)
                    ...
                    return false;
                if (result.Start < formulaRange.Start
                    || result.End > formulaRange.End)
                    return false;
                matched = true;
            }
            if (!matched || result is null)
                return false;
    ...
    internal static void NormalizePreservedVisualTeXNumberedTabParagraph(
        Document document,
        Range formulaRange,
        float formulaHeightPoints,
        FormulaMetadata metadata)
    {
        ...
        try
        {
            if (RangeIsWhollyWithinTable(formulaRange))
                throw new InvalidDataException(
                    "A preserved VisualTeX body tab paragraph cannot be inside a Word table.");
            if (formulaRange.Start <= document.Content.Start)
                throw new InvalidDataException(
                    "A preserved VisualTeX numbered OLE lost its leading center-tab slot.");
            ...
            if (!string.Equals(
                    ...
                    StringComparison.Ordinal))
                throw new InvalidDataException(
                    "A preserved VisualTeX numbered OLE lost the leading center-tab character.");
            ...
            if (maths.Count != 0
                || shapes.Count != 1)
                throw new InvalidDataException(
                    "The preserved VisualTeX tab host no longer contains exactly one OLE formula.");

            // Replacing an InlineShape can make Word reapply the ordinary block
            // writer's centered paragraph format even though the surrounding
            // center/right-tab numbering scaffold stayed intact. Restore only the
            // canonical paragraph geometry here; SEQ/REF fields and text ranges are
            // deliberately untouched.
            ConfigureEquationParagraph(
                formulaRange,
                numbered: true);
            AlignDisplayOleObjectToParagraphBaseline(
                document,
            ...
            // Replacing an InlineShape at the left edge of a preserved tab host
            // makes Word grow the existing VTEq_* bookmark backwards so it owns
            // the replacement OLE as well as the visible TAB/(REF) label. The
            // SEQ target and local REF remain healthy; repair only that convenience
            // bookmark from the proven local canonical label after the new OLE is
            // fully materialized.
            if (!TryRebindHealthyBodyTabVisibleNumber(
                    document,
                    formulaRange,
                    metadata.FormulaId,
                    NativeNumberBookmarkName(metadata.FormulaId)))
                throw new InvalidDataException(
                    "The preserved VisualTeX tab host lost its exact local equation-number label after OLE replacement.");
        }
        finally
        {
            Release(shapes);
    ...
    internal static void ConfigureVisualTeXDisplayHostLayout(
        Document document,
        Range formulaRange,
        float formulaHeightPoints,
        FormulaMetadata metadata)
    {
        ...
        if (formulaRange is null)
            throw new ArgumentNullException(nameof(formulaRange));
        if (metadata is null)
        ...
        if (!string.Equals(
                ...
                StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException(
                "VisualTeX display-host layout requires block metadata.");
        if (ContainsNativeOmml(formulaRange))
            throw new InvalidDataException(
                "VisualTeX OLE display-host layout cannot be applied to native OMML.");
        ...
        try
        {
            ...
            if (paragraphs.Count != 1)
                throw new InvalidDataException(
                    "A VisualTeX display host must belong to exactly one Word paragraph.");
            paragraph = paragraphs[1];
            format = paragraph.Format;

            // Keep the existing paragraph's line-spacing/font contract intact.
            // Only the display scaffold itself is canonicalized here: VisualTeX
            // display OLEs, numbered or not, use the same justified paragraph
            // with center/right tabs. The unnumbered form simply has no trailing
            // number label.
            format.Alignment =
        ...
        // Reuse the exact tab geometry, leading-tab ownership and OLE baseline
        // primitives used by VisualTeX equation numbering. This prevents fresh
        // unnumbered insertion from drifting back to the retired centered-host
        // model before any numbering operation has occurred.
        ConfigureEquationParagraphTabStops(
            formulaRange,
            numbered: true);
        EnsureLeadingEquationTab(
            document,
            formulaRange);
        AlignDisplayOleObjectToParagraphBaseline(
            document,
        ...
        try
        {
            ...
            if (!nativeOmmlParagraph)
            {
                ...
                format.BaseLineAlignment =
                    (WdBaselineAlignment)(int)WordInlineHostAlignment.Automatic;
            }
            ...
            var leftMargin = 72f;
            var rightMargin = 72f;
            var hasCellWidth = TryResolveOwningTableCellContentWidth(
                formulaRange,
                out var cellContentWidth);
            if (!hasCellWidth)
            {
                try
                {
            ...
            var positions = hasCellWidth
                ? (Center: cellContentWidth / 2f, Right: cellContentWidth)
                : CalculateEquationTabStops(pageWidth, leftMargin, rightMargin, 0, 0);
            if (!numbered && nativeOmmlParagraph)
            {
                // Standalone native OMML remains Word-native display math. The
                // tab-driven host below is specifically the VisualTeX OLE layout
                // contract and must not be imposed on Word's own OMath paragraph.
                format.Alignment = WdParagraphAlignment.wdAlignParagraphCenter;
                return;
            }
    ...
    private static void ConfigureEquationParagraphTabStops(
        Range formulaRange,
        bool numbered)
    {
        Paragraphs? paragraphs = null;
        ...
        try
        {
            ...
            if (paragraphs.Count != 1)
                throw new InvalidDataException(
                    "VisualTeX equation tab geometry requires one paragraph.");
            paragraph = paragraphs[1];
            paragraphRange = paragraph.Range.Duplicate;
            format = paragraph.Format;
            tabStops = format.TabStops;
            ...
            if (!hasCellWidth)
            {
                ...
                {
                    // Keep the same safe page fallback as the full paragraph
                    // configurator for protected/custom stories.
                }
            }

            var positions = hasCellWidth
                ? (Center: cellContentWidth / 2f, Right: cellContentWidth)
                : CalculateEquationTabStops(
                    pageWidth,
                    leftMargin,
                    rightMargin,
                    0,
        ...
        bool knownFreshConversionTarget = false)
    {
        ...
        Range? captionRange = null;
        Range? numberRange = null;
        if (!knownFreshConversionTarget
            && TryGetNativeCaptionRanges(
                document,
                formulaId,
                nativeSequenceName,
                out captionRange,
    ...
    private static bool TryCaptureTopLevelTableAt(
        Document document,
        int position,
        ...
        out int tableEnd)
    {
        ...
        try
        {
            ...
            for (var index = 1; index <= tables.Count; index++)
            {
                Release(tableRange); tableRange = null;
                Release(table); table = tables[index];
                if (table.NestingLevel != 1) continue;
                tableRange = table.Range;
                if (tableRange.StoryType != storyType || tableRange.Start != position)
                    continue;
    ...
    private static bool TryCaptureImmediateFollowingTopLevelTableBoundary(
        Document document,
        Range paragraphOwnedRange,
        out int tableStart,
        out int tableEnd)
    {
        tableStart = -1;
        ...
        try
        {
            paragraphs = paragraphOwnedRange.Paragraphs;
            if (paragraphs.Count != 1) return false;
            paragraph = paragraphs[1];
            paragraphRange = paragraph.Range;
            tableStart = paragraphRange.End;
            if (!TryCaptureTopLevelTableAt(
                    document,
                    tableStart,
                    paragraphRange.StoryType,
                    out tableEnd))
            {
                tableStart = -1;
                tableEnd = -1;
    ...
    internal static bool IsExactOwnedNativeCaptionFlowTail(
        int expectedStart,
        int rangeStart,
        int rangeEnd,
        string? text,
    ...
    private static void MarkGeneratedNativeCaptionFlowTail(
        Document document,
        string formulaId,
        int followingTableStartBeforeStyle,
        int followingTableEndBeforeStyle)
    {
        Range? tail = null;
        Paragraphs? tailParagraphs = null;
        Paragraph? tailParagraph = null;
        Range? tailParagraphRange = null;
        InlineShapes? shapes = null;
        OMaths? maths = null;
        Fields? fields = null;
        Frames? frames = null;
        Tables? localTables = null;
        ...
        try
        {
            ...
            if (!bookmarks.Exists(captionName))
                throw new InvalidDataException(
                    "The native caption identity disappeared while its Frame was created.");

            if (TryCaptureTopLevelTableAt(
                    document,
                    followingTableStartBeforeStyle,
                    WdStoryType.wdMainTextStory,
                    out var unchangedTableEnd))
            {
                if (unchangedTableEnd != followingTableEndBeforeStyle)
                    throw new InvalidDataException(
                        "The following user table changed while styling the native caption.");
                return; // This Word build did not materialize an extra flow paragraph.
            }

            tail = document.Range(
                followingTableStartBeforeStyle,
                followingTableStartBeforeStyle + 1);
            tailParagraphs = tail.Paragraphs;
            if (tailParagraphs.Count != 1)
                throw new InvalidDataException(
                    "The generated native-caption flow tail has ambiguous paragraph ownership.");
            tailParagraph = tailParagraphs[1];
            tailParagraphRange = tailParagraph.Range;
            shapes = tail.InlineShapes;
            maths = tail.OMaths;
            fields = WordFormulaHost.GetLocalFields(tail);
            frames = tail.Frames;
            localTables = tail.Tables;
            localBookmarks = tail.Bookmarks;
            var onlyOwnedTransientAliases = true;
            for (var bookmarkIndex = 1; bookmarkIndex <= localBookmarks.Count; bookmarkIndex++)
            {
                Release(localBookmark); localBookmark = localBookmarks[bookmarkIndex];
                if (!string.Equals(localBookmark.Name, captionName, StringComparison.OrdinalIgnoreCase)
                    && !string.Equals(localBookmark.Name, numberName, StringComparison.OrdinalIgnoreCase))
                {
                    onlyOwnedTransientAliases = false;
                    break;
                }
            }
            if (!IsExactOwnedNativeCaptionFlowTail(
                    followingTableStartBeforeStyle,
                    tail.Start,
                    tail.End,
                    tail.Text,
                    tailParagraphRange.Start,
                    tailParagraphRange.End,
                    RangeIsWhollyWithinTable(tail),
                    localTables.Count,
                    shapes.Count,
                    maths.Count,
                    fields.Count,
                    frames.Count)
                || !onlyOwnedTransientAliases)
                throw new InvalidDataException(
                    "Word inserted unexpected content between the native caption and following table.");

            if (!TryCaptureTopLevelTableAt(
                    document,
                    tail.End,
                    tail.StoryType,
                    out var shiftedTableEnd)
                || shiftedTableEnd != followingTableEndBeforeStyle + 1)
                throw new InvalidDataException(
                    "The following user table did not shift by exactly one generated caption-flow paragraph.");

            var name = NativeCaptionFlowTailBookmarkName(formulaId);
            if (bookmarks.Exists(name))
                throw new InvalidDataException(
                    "A native-caption flow-tail identity already exists before creation.");
            marker = bookmarks.Add(name, tail);
            marked = marker.Range;
            if (marked.Start != tail.Start
                || marked.End != tail.End
                || !string.Equals(marked.Text, "\r", StringComparison.Ordinal))
                throw new InvalidDataException(
                    "Word did not retain the generated native-caption flow-tail identity.");
            WordDoubleClickHook.TraceMessage(
                $"native-caption-flow-tail-owned formulaId={formulaId} range={tail.Start}:{tail.End}");
        }
        finally
        {
            Release(marked);
            ...
            Release(maths);
            Release(shapes);
            Release(tailParagraphRange);
            Release(tailParagraph);
            Release(tailParagraphs);
            Release(tail);
        }
    }

    internal static bool IsEmptyOwnedCaptionFlowTailBeforeConversion(Document document, string formulaId)
    {
        Bookmarks? bookmarks = null;
        Bookmark? marker = null;
        Range? range = null;
        ...
        try
        {
            ...
            if (range.End != range.Start + 1 || range.Text != "\r"
                || RangeIsWhollyWithinTable(range)) return false;
            paragraphs = range.Paragraphs;
            if (paragraphs.Count != 1) return false;
            paragraph = paragraphs[1];
            paragraphRange = paragraph.Range;
            return paragraphRange.Start == range.Start && paragraphRange.End == range.End
                && TryCaptureTopLevelTableAt(document, range.End, range.StoryType, out _);
        }
        finally { Release(paragraphRange); Release(paragraph); Release(paragraphs); Release(range); Release(marker); Release(bookmarks); }
    }

    internal static void CompleteDeferredCaptionFlowTailCleanup(
        Document document, string sourceFormulaId, bool verifiedEmptyBeforeConversion)
        => CleanupOwnedNativeCaptionFlowTail(document, sourceFormulaId, deleteParagraph: true,
            recoverExpandedStart: verifiedEmptyBeforeConversion);

    internal static void CompleteDeferredOmmlSeparatorCleanup(Document document, string sourceFormulaId)
        => CleanupOwnedNativeCaptionFlowTail(document, sourceFormulaId, deleteParagraph: true,
            recoverExpandedStart: true,
            ownedMarkerName: NativeOmmlOwnedSeparatorBookmarkName(sourceFormulaId));

    private static void CleanupOwnedNativeCaptionFlowTail(
        Document document,
        string formulaId,
        bool deleteParagraph,
        bool recoverExpandedStart = false,
        string? ownedMarkerName = null)
    {
        Bookmarks? bookmarks = null;
        Bookmark? marker = null;
        Range? range = null;
        ...
        Range? content = null;
        Range? previous = null;
        var tableEndBeforeDelete = -1;
        var safeToDelete = false;
        try
        {
            bookmarks = document.Bookmarks;
            var name = ownedMarkerName ?? NativeCaptionFlowTailBookmarkName(formulaId);
            if (!bookmarks.Exists(name)) return;
            marker = bookmarks[name];
            range = marker.Range;
            // A freshly inserted OLE can expand the START of the next paragraph's
            // bookmark backward. Only a transaction which proved this exact tag
            // owned one empty paragraph before mutation may recover its surviving
            // end. Persisted expanded/user-edited tags never receive this authority.
            if (recoverExpandedStart && range.End > range.Start + 1)
            {
                var survivingEnd = range.End;
                Release(range);
                range = document.Range(survivingEnd - 1, survivingEnd);
            }
            paragraphs = range.Paragraphs;
            if (paragraphs.Count == 1)
            {
                ...
                frames = range.Frames;
                localTables = range.Tables;
                localBookmarks = paragraphRange.Bookmarks;
                var ownsOnlyMarker = localBookmarks.Count == 1;
                if (ownsOnlyMarker)
                {
                    localBookmark = localBookmarks[1];
                    ownsOnlyMarker = string.Equals(
                        localBookmark.Name,
                        name,
                        StringComparison.OrdinalIgnoreCase);
                }
                content = document.Content;
                var previousIsTable = false;
                if (paragraphRange.Start > content.Start)
                {
                    previous = document.Range(
                        paragraphRange.Start - 1,
                        paragraphRange.Start);
                    previousIsTable = RangeIsWhollyWithinTable(previous);
                }
                var followedByTopLevelTable = TryCaptureTopLevelTableAt(
                    document,
                    paragraphRange.End,
                    paragraphRange.StoryType,
                    out tableEndBeforeDelete);
                safeToDelete = ownsOnlyMarker
                    && !previousIsTable
                    && followedByTopLevelTable
                    && IsExactOwnedNativeCaptionFlowTail(
                        range.Start,
                        range.Start,
                        range.End,
                        range.Text,
                        paragraphRange.Start,
                        paragraphRange.End,
                        RangeIsWhollyWithinTable(range),
                        localTables.Count,
                        shapes.Count,
                        maths.Count,
                        fields.Count,
            ...
            if (Environment.GetEnvironmentVariable("VISUALTEX_NUMBERED_PERF_TRACE") == "1")
                WordDoubleClickHook.TraceMessage(
                    $"native-caption-flow-tail-check formulaId={formulaId} range={range.Start}:{range.End} text=[{(range.Text ?? string.Empty).Replace("\r", "<CR>")}] paragraphs={paragraphs.Count} paragraph={paragraphRange?.Start}:{paragraphRange?.End} tables={localTables?.Count} shapes={shapes?.Count} maths={maths?.Count} fields={fields?.Count} frames={frames?.Count} bookmarks={localBookmarks?.Count} safe={safeToDelete}");
            marker.Delete();
            Release(marker); marker = null;
            if (!deleteParagraph || !safeToDelete)
            {
                WordDoubleClickHook.TraceMessage(
                    $"native-caption-flow-tail-released formulaId={formulaId} deleteRequested={deleteParagraph} safe={safeToDelete}");
                return;
            }

            var oldStart = paragraphRange!.Start;
            paragraphRange.Delete();
            if (!TryCaptureTopLevelTableAt(
                    document,
                    oldStart,
                    WdStoryType.wdMainTextStory,
                    out var tableEndAfterDelete)
                || tableEndAfterDelete != tableEndBeforeDelete - 1)
                throw new InvalidDataException(
                    "Removing the owned native-caption flow tail changed the following user table unexpectedly.");
            WordDoubleClickHook.TraceMessage(
                $"native-caption-flow-tail-removed formulaId={formulaId} start={oldStart}");
        }
        finally
        {
            Release(previous);
    ...
    private static void CaptureBodyCaptionFollowingWitness(
        Document document,
        Range captionRange,
        out int followingStart,
        out int contentEnd,
        out int witnessSpan,
        out string witness)
    {
        ...
        {
            content = document.Content;
            followingStart = Math.Max(
                content.Start,
                Math.Min(captionRange.End, content.End));
            contentEnd = content.End;
            var witnessEnd = Math.Min(
                content.End,
                followingStart + 64);
            witnessSpan = Math.Max(
                0,
                witnessEnd - followingStart);
            if (witnessSpan == 0)
            {
            ...
            probe = document.Range(
                followingStart,
                witnessEnd);
            witness = probe.Text ?? string.Empty;
        }
        finally
        {
            Release(probe);
    ...
    private static bool TryRemoveGeneratedBodyCaptionFlowParagraph(
        Document document,
        string formulaId,
        int followingStartBeforeStyle,
        int contentEndBeforeStyle,
        int followingWitnessSpan,
        string followingWitness)
    {
        Range? content = null;
        ...
        Tables? tables = null;
        Bookmarks? bookmarks = null;
        Range? shiftedWitness = null;
        Range? restoredWitness = null;
        try
        {
            content = document.Content;
            if (content.End != contentEndBeforeStyle + 1
                || followingStartBeforeStyle < content.Start
                || followingStartBeforeStyle + 1 > content.End)
            {
                return false;
            }

            candidate = document.Range(
                followingStartBeforeStyle,
                followingStartBeforeStyle + 1);
            ...
            fields = WordFormulaHost.GetLocalFields(candidate);
            frames = candidate.Frames;
            tables = candidate.Tables;
            bookmarks = candidate.Bookmarks;
            var exactGeneratedTail = IsExactOwnedNativeCaptionFlowTail(
                    followingStartBeforeStyle,
                    candidate.Start,
                    candidate.End,
                    candidate.Text,
                    paragraphRange.Start,
                    paragraphRange.End,
                    RangeIsWhollyWithinTable(candidate),
                    tables.Count,
                    shapes.Count,
                    maths.Count,
                    fields.Count,
                    frames.Count);
            var onlyOwnedAliases = true;
            Bookmark? localBookmark = null;
            try
            {
                var captionName = NativeCaptionBookmarkName(formulaId);
                ...
                for (var bookmarkIndex = 1;
                     bookmarkIndex <= bookmarks.Count;
                     bookmarkIndex++)
                {
                    Release(localBookmark);
                    localBookmark = bookmarks[bookmarkIndex];
                    if (!string.Equals(
                            localBookmark.Name,
                            captionName,
                            StringComparison.OrdinalIgnoreCase)
                        && !string.Equals(
                            ...
                            StringComparison.OrdinalIgnoreCase))
                    {
                        onlyOwnedAliases = false;
                        break;
                    }
                }
            }
            finally { Release(localBookmark); }
            if (!exactGeneratedTail
                || !onlyOwnedAliases)
                return false;

            if (followingWitnessSpan == 0
                && followingStartBeforeStyle == contentEndBeforeStyle)
            {
                // A Word document must always retain one final body paragraph.
                // When a clipping Frame is created for the final equation caption,
                // Word materializes that mandatory trailing CR outside the Frame.
                // Deleting it simply makes Word recreate the same paragraph, so
                // preserve the structural terminator and compress only its visual
                // line box. This is safe only for the exact empty, object-free,
                // bookmark-free/owned-alias tail proven above.
                CompressNativeCaptionParagraphFlow(candidate);
                return true;
            }
            ...
            if (followingWitnessSpan > 0)
            {
                var shiftedEnd = Math.Min(
                    ...
                    followingStartBeforeStyle
                    + 1
                    + followingWitnessSpan);
                shiftedWitness = document.Range(
                    followingStartBeforeStyle + 1,
                    shiftedEnd);
                if (!string.Equals(
                        shiftedWitness.Text ?? string.Empty,
                        followingWitness,
                        StringComparison.Ordinal))
                {
                    return false;
            ...
            Release(content);
            content = document.Content;
            if (content.End != contentEndBeforeStyle)
                throw new InvalidDataException(
                    "Removing Word's generated equation-caption flow paragraph changed the document length unexpectedly.");

            if (followingWitnessSpan > 0)
            {
                var restoredEnd = Math.Min(
                    content.End,
                    followingStartBeforeStyle
                    + followingWitnessSpan);
                restoredWitness = document.Range(
                    followingStartBeforeStyle,
                    restoredEnd);
                if (!string.Equals(
                        restoredWitness.Text ?? string.Empty,
                        followingWitness,
                        StringComparison.Ordinal))
                    throw new InvalidDataException(
                        "Removing Word's generated equation-caption flow paragraph changed following user content.");
            }
            return true;
        }
        finally
        {
            Release(restoredWitness);
            Release(shiftedWitness);
            Release(bookmarks);
            Release(tables);
            Release(frames);
            Release(fields);
    ...
    private static int CreateNativeCaptionParagraphAfterBodyFormula(
        Document document,
        Range formulaParagraphRange)
    {
        Range? content = null;
        ...
        Range? insertion = null;
        Range? verification = null;
        Cell? verificationCell = null;
        try
        {
            content = document.Content;
            var captionStart = formulaParagraphRange.End;
            var followedImmediatelyByTable = false;
            if (captionStart < content.End)
            {
                followingProbe = document.Range(
                    captionStart,
                    Math.Min(content.End, captionStart + 1));
                ...
                if (followingTables.Count > 0)
                {
                    ...
                    followedImmediatelyByTable =
                        followingTableRange.StoryType == formulaParagraphRange.StoryType
                        && followingTableRange.Start == captionStart;
                }
            }

            if (!followedImmediatelyByTable)
            {
                formulaParagraphRange.InsertParagraphAfter();
                return captionStart;
            }

            // Word 2021 treats Range.InsertParagraphAfter() at a body→table
            // boundary as an insertion into cell (1,1). The actual body boundary
            // is the preceding paragraph mark. Insert a NEW mark immediately before
            // that existing mark: the formula keeps the new mark, the old mark
            // becomes one empty body paragraph, and the whole table moves forward
            // by one story position without changing any cell.
            var terminatorLength =
                GetWordParagraphStructuralTerminatorLength(formulaParagraphRange);
            if (terminatorLength != 1
                || formulaParagraphRange.End - terminatorLength < formulaParagraphRange.Start)
                throw new InvalidDataException(
                    "The body equation before a Word table has no ordinary paragraph terminator.");
            insertion = document.Range(
                formulaParagraphRange.End - terminatorLength,
                formulaParagraphRange.End - terminatorLength);
            insertion.InsertBefore("\r");

            verification = document.Range(captionStart, captionStart + 1);
            if (!string.Equals(verification.Text, "\r", StringComparison.Ordinal))
                throw new InvalidDataException(
                    "Word did not create the dedicated equation-caption paragraph before the following table.");
            verificationCell = WordFormulaHost.TryGetOwningCell(verification);
            if (verificationCell is not null)
                throw new InvalidDataException(
                    "Word placed the dedicated equation-caption paragraph inside the following table.");

            Release(followingTableRange);
            followingTableRange = followingTable.Range;
            if (followingTableRange.Start != captionStart + 1)
                throw new InvalidDataException(
                    "Creating the equation-caption paragraph changed the following table boundary unexpectedly.");
            return captionStart;
        }
        finally
        {
            Release(verificationCell);
        ...
        bool allowNativeOmmlAcceptanceFixture = false)
    {
        ...
        Paragraph? paragraph = null;
        Bookmarks? bookmarks = null;
        var bodyCaption = false;
        var followingTableStartBeforeStyle = -1;
        var followingTableEndBeforeStyle = -1;
        try
        {
            ...
            else if (RangeIsWhollyWithinTable(formulaRange))
            {
                // All user-cell OLE captions live outside the enclosing table;
                // their visible REF stays beside the formula inside its own cell.
                // InsertParagraphAfter on a Cell paragraph would target the next
                // cell's content and is never a valid caption insertion boundary.
                formulaTable = formulaRange.Tables[1];
                formulaTableRange = formulaTable.Range;
                captionStart = ResolveCaptionAppendPositionAfterUserTable(document, formulaTableRange.End);
                if (!CanReuseEmptyNativeCaptionParagraph(document, captionStart))
                {
                    fieldInsertion = document.Range(captionStart, captionStart);
                    fieldInsertion.InsertParagraphBefore();
            ...
            else
            {
                ...
                captionStart = CreateNativeCaptionParagraphAfterBodyFormula(
                    document,
                    formulaParagraphRange);
            }
            ...
            if (bodyCaption)
            {
                TryCaptureImmediateFollowingTopLevelTableBoundary(
                    document,
                    captionRange,
                    out followingTableStartBeforeStyle,
                    out followingTableEndBeforeStyle);
            }
            TraceStage("bookmarks");
            var bodyFlowStartBeforeStyle = -1;
            var bodyContentEndBeforeStyle = -1;
            var bodyFollowingWitnessSpan = 0;
            var bodyFollowingWitness = string.Empty;
            if (bodyCaption
                && followingTableStartBeforeStyle < 0)
            {
                CaptureBodyCaptionFollowingWitness(
                    document,
                    captionRange,
                    out bodyFlowStartBeforeStyle,
                    out bodyContentEndBeforeStyle,
                    out bodyFollowingWitnessSpan,
                    out bodyFollowingWitness);
            }
            if (deferFieldUpdate
            ...
            if (bodyCaption
                && followingTableStartBeforeStyle < 0
                && bodyFlowStartBeforeStyle >= 0)
            {
                _ = TryRemoveGeneratedBodyCaptionFlowParagraph(
                    document,
                    formulaId,
                    bodyFlowStartBeforeStyle,
                    bodyContentEndBeforeStyle,
                    bodyFollowingWitnessSpan,
                    bodyFollowingWitness);
            }
            if (bodyCaption && followingTableStartBeforeStyle >= 0)
            {
                MarkGeneratedNativeCaptionFlowTail(
                    document,
                    formulaId,
                    followingTableStartBeforeStyle,
                    followingTableEndBeforeStyle);
            }
    ...
    private static int ResolveCaptionAppendPositionAfterUserTable(Document document, int tableEnd)
    {
        Range? probe = null;
        Frames? frames = null;
        Frame? frame = null;
        ...
        Paragraphs? paragraphs = null;
        Paragraph? paragraph = null;
        Range? lastParagraph = null;
        Bookmarks? bookmarks = null;
        Bookmark? bookmark = null;
        Range? bookmarked = null;
        try
        {
            if (tableEnd >= document.Content.End) return tableEnd;
            probe = document.Range(tableEnd, tableEnd + 1);
            frames = probe.Frames;
            if (frames.Count != 1) return tableEnd;
            frame = frames[1];
            framed = frame.Range;
            if (framed.Start != tableEnd || framed.End <= tableEnd
                || frame.Width > 1f || frame.Height > 1f || !frame.LockAnchor || frame.TextWrap)
                return tableEnd;
            // Word coalesces identically formatted clipping captions into one
            // Frame. Append after its final paragraph, never before the first
            // caption: prepending at Table.End drags all existing number aliases
            // onto the new SEQ field through bookmark insertion gravity.
            probe.SetRange(framed.End - 1, framed.End);
            paragraphs = probe.Paragraphs;
            paragraph = paragraphs[1];
            lastParagraph = paragraph.Range;
            bookmarks = lastParagraph.Bookmarks;
            for (var index = 1; index <= bookmarks.Count; index++)
            {
                Release(bookmarked); bookmarked = null;
                Release(bookmark); bookmark = bookmarks[index];
                if (!bookmark.Name.StartsWith(NativeCaptionBookmarkPrefix, StringComparison.Ordinal)
                    || !Guid.TryParseExact(bookmark.Name.Substring(NativeCaptionBookmarkPrefix.Length), "N", out _))
                    continue;
                bookmarked = bookmark.Range;
                if (bookmarked.StoryType == lastParagraph.StoryType
                    && WordFormulaHost.ContainsPhysicalRange(lastParagraph.Start, lastParagraph.End,
                        bookmarked.Start, bookmarked.End))
                    return lastParagraph.End;
            }
            throw new InvalidDataException("The existing clipped equation-caption block has lost its independent final identity; no new caption was inserted.");
        }
        finally
        {
            Release(bookmarked); Release(bookmark); Release(bookmarks);
            Release(lastParagraph); Release(paragraph); Release(paragraphs);
            Release(framed); Release(frame); Release(frames); Release(probe);
        }
    }
    ...
    private static Range GetNativeCaptionFrameContentRange(Range captionRange)
    {
        Range? frameRange = null;
        Cell? cell = null;
        try
        {
            frameRange = captionRange.Duplicate;
            var terminatorLength = GetWordParagraphStructuralTerminatorLength(frameRange);
            if (terminatorLength <= 0
                || frameRange.End - terminatorLength <= frameRange.Start)
                throw new InvalidDataException(
                    "The native equation caption has no frameable paragraph content.");
            frameRange.End -= terminatorLength;
            cell = WordFormulaHost.TryGetOwningCell(frameRange);
            if (cell is not null)
                throw new InvalidDataException(
                    "A native equation caption unexpectedly entered a user table cell.");
            var result = frameRange;
            frameRange = null;
            return result;
        }
        finally
        ...
        Range numberRange)
    {
        ...
        try
        {
            ...
            // Keep the final paragraph mark in normal body flow. Word 2021 rejects
            // Frames.Add on a whole caption paragraph immediately before a user
            // table because the paragraph terminator has table-boundary affinity.
            frameRange = GetNativeCaptionFrameContentRange(captionRange);
            frames = frameRange.Frames;
            frame = frames.Count > 0 ? frames[1] : frames.Add(frameRange);
        ...
        bool cleanupLegacyFrames = true)
    {
        ...
        try
        {
            ...
            // Frame the SEQ/prefix contents only. The paragraph mark must remain
            // outside the Frame, especially at the body-to-user-table boundary.
            frameRange = GetNativeCaptionFrameContentRange(captionRange);
            frames = frameRange.Frames;
            frame = frames.Count > 0 ? frames[1] : frames.Add(frameRange);
        ...
        bool knownFreshConversionTarget = false)
    {
        var targetBookmarkName = NativeNumberBookmarkName(formulaId);
        if (!knownFreshConversionTarget
            && knownNumberedTable is null
            && !IsNumberedEquationTable(formulaRange)
            && TryRebindHealthyBodyTabVisibleNumber(
                document,
                formulaRange,
                formulaId,
                targetBookmarkName))
    ...
    private static bool TryRebindHealthyBodyTabVisibleNumber(
        Document document,
        Range formulaRange,
        string formulaId,
        string targetBookmarkName)
    {
        ...
        {
            if (ContainsNativeOmml(formulaRange)
                || RangeIsWhollyWithinTable(formulaRange))
                return false;
            ...
            var editableEnd = Math.Max(
                paragraphRange.Start,
                paragraphRange.End - 1);
            if (formulaRange.End < paragraphRange.Start
            ...
            if (!labelText.StartsWith("\t(", StringComparison.Ordinal)
                ...
                || labelText.IndexOf('\r') >= 0
                || labelText.IndexOf('\a') >= 0)
                return false;

            fields = WordFormulaHost.GetLocalFields(labelRange);
            var targetReferenceCount = 0;
            for (var index = 1; index <= fields.Count; index++)
            {
                Release(code);
                code = null;
            ...
            {
                existing = bookmarks[visibleName];
                var existingRange = existing.Range;
                try
                {
                    if (existingRange.StoryType == labelRange.StoryType
                        && existingRange.Start == labelRange.Start
                        && existingRange.End == labelRange.End
                        && string.Equals(
                            existingRange.Text,
                            labelText,
                            StringComparison.Ordinal))
                ...
                finally { Release(existingRange); }
                existing.Delete();
                Release(existing);
                existing = null;
            }

            rebound = bookmarks.Add(
                visibleName,
                labelRange);
            return true;
        }
    ...
    internal static void RebindFreshVisibleEquationNumberBookmark(
        Document document,
        Range formulaRange,
        string formulaId)
    {
        ...
        Range? exactLabel = null;
        Range? editableLabel = null;
        Range? nextParagraphPrefix = null;
        Range? suffixInsertion = null;
        Bookmark? rebound = null;
        try
        {
            bookmarks = document.Bookmarks;
            ...
            if (!bookmarks.Exists(visibleName))
                throw new InvalidDataException(
                    $"Fresh VisualTeX number {formulaId} lost its visible bookmark.");
            visibleBookmark = bookmarks[visibleName];
            visibleRange = visibleBookmark.Range;
            var labelStart = visibleRange.Start;

            paragraphs = formulaRange.Paragraphs;
            if (paragraphs.Count != 1)
                throw new InvalidDataException(
                    $"Fresh VisualTeX number {formulaId} has no single formula paragraph.");
            paragraph = paragraphs[1];
            paragraphRange = paragraph.Range.Duplicate;
            fields = WordFormulaHost.GetLocalFields(paragraphRange);
            var targetBookmark = NativeNumberBookmarkName(formulaId);
            for (var index = 1; index <= fields.Count; index++)
            {
                Release(resultRange);
                resultRange = null;
                ...
                if (field.Type != WdFieldType.wdFieldRef)
                    continue;
                code = field.Code;
                if (!IsReferenceToBookmark(code.Text, targetBookmark))
                    continue;
            ...
            if (resultRange is null)
                throw new InvalidDataException(
                    $"Fresh VisualTeX number {formulaId} has no local REF result.");

            if (labelStart < paragraphRange.Start
                || labelStart >= paragraphRange.End)
                throw new InvalidDataException(
                    $"Fresh VisualTeX number {formulaId} has an invalid visible label start.");

            var editableEnd = Math.Max(
                paragraphRange.Start,
                paragraphRange.End - 1);
            editableLabel = document.Range(labelStart, editableEnd);
            var editableText = editableLabel.Text ?? string.Empty;
            if (!editableText.StartsWith("\t(", StringComparison.Ordinal))
                throw new InvalidDataException(
                    $"Fresh VisualTeX number {formulaId} lost its TAB/'(' prefix.");

            if (!editableText.EndsWith(")", StringComparison.Ordinal))
            {
                // Deferred REF updates can either push the generated ')' across the
                // paragraph mark or consume it while replacing an initially-empty
                // field result. Repair only a proven fresh scaffold: TAB + '(' must
                // be followed by exactly this local REF result, with no user text
                // between the result and the paragraph terminator.
                var resultText = resultRange.Text ?? string.Empty;
                var expectedWithoutSuffix = "\t(" + resultText;
                if (!string.Equals(
                        editableText,
                        expectedWithoutSuffix,
                        StringComparison.Ordinal))
                    throw new InvalidDataException(
                        $"Fresh VisualTeX number {formulaId} lost its closing parenthesis "
                        + "but the remaining paragraph is not exactly TAB + (REF).");

                var movedSuffix = false;
                Range? content = null;
                try
                {
                    content = document.Content;
                    if (paragraphRange.End < content.End)
                    {
                        nextParagraphPrefix = document.Range(
                            paragraphRange.End,
                            Math.Min(content.End, paragraphRange.End + 1));
                        if (string.Equals(
                                nextParagraphPrefix.Text,
                                ")",
                                StringComparison.Ordinal))
                        {
                            nextParagraphPrefix.Delete();
                            movedSuffix = true;
                        }
                    }
                }
                ...
                Release(paragraphRange);
                paragraphRange = paragraph.Range.Duplicate;
                editableEnd = Math.Max(
                    paragraphRange.Start,
                    paragraphRange.End - 1);
                Release(resultRange);
                resultRange = field.Result.Duplicate;

                // Whether ')' was moved or consumed, materialize it only after the
                // deferred REF result is now final. Use the paragraph's editable
                // end, not Result.End, because Word 2021 can report Result.End at
                // the structural paragraph position.
                var suffixPosition = editableEnd;
                if (suffixPosition <= labelStart + 1)
                    throw new InvalidDataException(
                        $"Fresh VisualTeX number {formulaId} has no legal suffix insertion point.");
                suffixInsertion = document.Range(
                    suffixPosition,
                    suffixPosition);
                suffixInsertion.Text = ")";

                if (movedSuffix)
                {
                    // The old generated suffix was removed from the next paragraph
                    // above; no other content was touched.
                }
            }

            Release(editableLabel);
            editableLabel = null;
            Release(paragraphRange);
            paragraphRange = paragraph.Range.Duplicate;
            editableEnd = Math.Max(
                paragraphRange.Start,
                paragraphRange.End - 1);
            exactLabel = document.Range(labelStart, editableEnd);
            var exactText = exactLabel.Text ?? string.Empty;
            if (!exactText.StartsWith("\t(", StringComparison.Ordinal)
                || !exactText.EndsWith(")", StringComparison.Ordinal)
                || exactText.IndexOf('\r') >= 0
                || exactText.IndexOf('\a') >= 0
                || exactLabel.Paragraphs.Count != 1)
                throw new InvalidDataException(
                    $"Fresh VisualTeX number {formulaId} did not normalize to one TAB/(REF) label.");

            var exactStart = exactLabel.Start;
            var exactEnd = exactLabel.End;
            // Field.Result.End is not a visible-text boundary in Word 2021. For a
            // REF at paragraph end it may resolve at the paragraph-structure
            // position even though Result.Text is wholly inside TAB + (REF).
            // Prove the target REF locally, but keep VTEq_* bound only to the
            // visible label. The public range reader canonicalizes any bookmark
            // gravity that Word applies after Bookmarks.Add.
            if (resultRange.Start < exactStart
                || resultRange.Start >= exactEnd
                || resultRange.End > paragraphRange.End)
                throw new InvalidDataException(
                    $"Fresh VisualTeX number {formulaId} has a REF owner outside its visible label paragraph.");
            visibleBookmark.Delete();
            Release(visibleBookmark);
            visibleBookmark = null;

            // A Word Range is live. Deleting the old bookmark can expand a Range
            // that shared its boundary back across the paragraph mark. Freeze the
            // numeric boundary first, then reacquire a fresh Range only after the
            // old VTEq_* owner is gone.
            Release(exactLabel);
            exactLabel = document.Range(exactStart, exactEnd);
            var reboundSourceText = exactLabel.Text ?? string.Empty;
            if (!string.Equals(
                    reboundSourceText,
                    exactText,
                    StringComparison.Ordinal)
                || reboundSourceText.IndexOf('\r') >= 0
                || reboundSourceText.IndexOf('\a') >= 0
                || exactLabel.Paragraphs.Count != 1)
            {
                static string DescribeRebindCodes(string value) => string.Join(
                    ",",
                    value.Select(character => $"U+{(int)character:X4}"));
                throw new InvalidDataException(
                    $"Fresh VisualTeX number {formulaId} changed while rebinding its visible label. "
                    + $"range={exactStart}:{exactEnd}, "
                    + $"fieldResult={resultRange.Start}:{resultRange.End}, "
                    + $"source=[{reboundSourceText.Replace("\r", "<CR>").Replace("\a", "<CELL>").Replace("\t", "<TAB>")}], "
                    + $"expected=[{exactText.Replace("\r", "<CR>").Replace("\a", "<CELL>").Replace("\t", "<TAB>")}], "
                    + $"sourceCodes=[{DescribeRebindCodes(reboundSourceText)}], "
                    + $"expectedCodes=[{DescribeRebindCodes(exactText)}].");
            }
            rebound = bookmarks.Add(visibleName, exactLabel);
            Range? reboundRange = null;
            Range? canonicalRebound = null;
            try
            {
                reboundRange = rebound.Range;
                canonicalRebound = FindVisibleEquationNumberRange(
                    document,
                    formulaId)
                    ?? throw new InvalidDataException(
                        $"Fresh VisualTeX number {formulaId} lost its rebound visible label.");
                var canonicalText = canonicalRebound.Text ?? string.Empty;
                if (canonicalRebound.Start != exactLabel.Start
                    || !string.Equals(
                        canonicalText,
                        exactText,
                        StringComparison.Ordinal)
                    || canonicalRebound.Paragraphs.Count != 1)
                {
                    var rawText = reboundRange.Text ?? string.Empty;
                    static string DescribeCodes(string value) => string.Join(
                        ",",
                        value.Select(character => $"U+{(int)character:X4}"));
                    throw new InvalidDataException(
                        $"Fresh VisualTeX number {formulaId} could not rebind its exact visible label. "
                        + $"raw={reboundRange.Start}:{reboundRange.End}, "
                        + $"canonical={canonicalRebound.Start}:{canonicalRebound.End}, "
                        + $"expected={exactLabel.Start}:{exactLabel.End}, "
                        + $"fieldCode={code?.Start}:{code?.End}, "
                        + $"fieldResult={resultRange?.Start}:{resultRange?.End}, "
                        + $"canonicalParagraphs={canonicalRebound.Paragraphs.Count}, "
                        + $"rawText=[{rawText.Replace("\r", "<CR>").Replace("\a", "<CELL>").Replace("\t", "<TAB>")}], "
                        + $"canonicalText=[{canonicalText.Replace("\r", "<CR>").Replace("\a", "<CELL>").Replace("\t", "<TAB>")}], "
                        + $"expectedText=[{exactText.Replace("\r", "<CR>").Replace("\a", "<CELL>").Replace("\t", "<TAB>")}], "
                        + $"rawCodes=[{DescribeCodes(rawText)}], "
                        + $"canonicalCodes=[{DescribeCodes(canonicalText)}], "
                        + $"expectedCodes=[{DescribeCodes(exactText)}].");
                }
            }
            finally
            {
                Release(canonicalRebound);
                Release(reboundRange);
            }
        }
        finally
        {
            Release(rebound);
            Release(suffixInsertion);
            Release(nextParagraphPrefix);
            Release(editableLabel);
            Release(exactLabel);
            Release(resultRange);
            Release(code);
        ...
        bool knownFreshConversionTarget = false)
    {
        ...
        try
        {
            ...
            var conversionSafeBody =
                useConversionSafeVisibleNumber && !tableLayout;
            ...
            if (!tableLayout)
            {
                // Word 2021 can discard direct pPr tab-stop definitions while
                // inserting the REF field. Repair only TabStops here. Re-running
                // the full paragraph configurator at this point also rewrites the
                // field/result boundary and can pull the closing parenthesis or
                // paragraph mark into Field.Result.
                ConfigureEquationParagraphTabStops(
                    formulaRange,
                    numbered: true);
                TraceStage("final-tab-stops");
                Release(fieldResult);
                fieldResult = field.Result;
            }
            ...
            bookmarkRange = tableLayout && useConversionSafeVisibleNumber
                ? CompleteTableEquationNumberLabelAfterField(
                    ...
                    knownNumberedTable)
                : conversionSafeBody
                    ? CompleteParagraphEquationNumberLabelAfterField(
                        document,
                        formulaRange,
                        field,
                        labelStart)
    ...
    private static Range CompleteParagraphEquationNumberLabelAfterField(
        Document document,
        Range formulaRange,
        Field field,
        int labelStart)
    {
        ...
        try
        {
            ...
            if (paragraphs.Count != 1)
                throw new InvalidOperationException(
                    "The conversion-safe equation number no longer belongs to one paragraph.");
            paragraph = paragraphs[1];
            paragraphRange = paragraph.Range.Duplicate;
            var editableEnd = Math.Max(
                paragraphRange.Start,
                paragraphRange.End - 1);
            if (labelStart < paragraphRange.Start
                || labelStart >= editableEnd)
                throw new InvalidOperationException(
                    "The conversion-safe equation-number label start escaped its paragraph.");

            prefix = document.Range(
                labelStart,
                Math.Min(editableEnd, labelStart + 2));
            if (!string.Equals(prefix.Text, "\t(", StringComparison.Ordinal))
                throw new InvalidOperationException(
                    "The conversion-safe equation-number prefix is not TAB + '('.");

            liveResult = field.Result;
            if (liveResult.Start < paragraphRange.Start
                || liveResult.Start > editableEnd
                || liveResult.End > paragraphRange.End)
                throw new InvalidOperationException(
                    $"The conversion-safe REF result escaped its equation paragraph: "
                    + $"result={liveResult.Start}:{liveResult.End}, "
                    + $"paragraph={paragraphRange.Start}:{paragraphRange.End}.");
            var suffixPosition = Math.Min(
                Math.Max(labelStart + 2, liveResult.End + 1),
                editableEnd);
            if (suffixPosition <= labelStart + 1)
                throw new InvalidOperationException(
                    "The conversion-safe REF field has no legal closing-parenthesis position.");
            suffix = document.Range(suffixPosition, suffixPosition);
            suffix.Text = ")";

            Release(liveResult);
            liveResult = field.Result;
            if ((liveResult.Text ?? string.Empty)
            ...
            candidate = document.Range(
                labelStart,
                Math.Min(paragraphRange.End - 1, suffixPosition + 1));
            var text = candidate.Text ?? string.Empty;
            if (!text.StartsWith("\t(", StringComparison.Ordinal)
                || !text.EndsWith(")", StringComparison.Ordinal)
                || text.IndexOf('\r') >= 0
                || text.IndexOf('\a') >= 0)
                throw new InvalidOperationException(
                    "Word did not materialize one single-paragraph VisualTeX equation-number label.");
        ...
        Table? knownNumberedTable)
    {
        ...
        try
        {
            ...
            if (fallbackText.StartsWith(fallbackPrefix, StringComparison.Ordinal)
                ...
                && fallbackText.IndexOf('\r') < 0
                && fallbackText.IndexOf('\a') < 0)
            {
    ...
    private static void RemoveNativeCaption(
        ...
        bool deferFlowTailCleanup = false)
    {
        if (Environment.GetEnvironmentVariable("VISUALTEX_VSTO_TRACE_CAPTION_OWNERS") == "1")
        {
            var diagnosticBookmarks = document.Bookmarks;
            try
            {
                foreach (var diagnosticName in new[] { NativeCaptionBookmarkName(formulaId), NativeNumberBookmarkName(formulaId) })
                {
                    if (!diagnosticBookmarks.Exists(diagnosticName))
                    {
                        WordDoubleClickHook.TraceMessage($"caption-owner-probe name={diagnosticName} missing=True");
                        continue;
                    }
                    var diagnosticBookmark = diagnosticBookmarks[diagnosticName];
                    var diagnosticRange = diagnosticBookmark.Range;
                    try { WordDoubleClickHook.TraceMessage($"caption-owner-probe name={diagnosticName} range={diagnosticRange.Start}:{diagnosticRange.End} math={WordFormulaHost.IsWhollyInsideMath(diagnosticRange)} text=[{diagnosticRange.Text}]"); }
                    finally { Release(diagnosticRange); Release(diagnosticBookmark); }
                }
            }
            finally { Release(diagnosticBookmarks); }
        }
        if (TryRemoveNativeHashSequenceCaptionBookmarks(document, formulaId))
        {
            if (!deferFlowTailCleanup)
                CleanupOwnedNativeCaptionFlowTail(
                    document,
                    formulaId,
                    deleteParagraph: !preserveParagraphSeparator);
            return;
        }

        // Word bookmark insertion gravity can expand VTEqCap_<id> when the user
        // later inserts ordinary text/formulas exactly at the hidden caption's
        // former start boundary. VTEqNum_<id>, however, remains attached to the
        // actual SEQ result. Never trust a widened caption bookmark for deletion:
        // re-derive its ownership from the number bookmark's one verified SEQ
        // paragraph first. This keeps the overlap guard below strict while
        // preventing user content / unrelated OMath from being mistaken for
        // VisualTeX-owned caption content.
        ...
        // Conversion still needs a stable body paragraph while the replacement
        // object is inserted. Its transaction owns this marker until the new
        // target's physical host is verified; ordinary unnumbering is unchanged.
        if (!deferFlowTailCleanup)
            CleanupOwnedNativeCaptionFlowTail(
                document,
                formulaId,
    ...
    internal static void RebindExternalCaptionToNativeNumberParagraph(
        Document document,
        string formulaId)
    {
        Bookmarks? bookmarks = null;
        ...
        try
        {
            bookmarks = document.Bookmarks;
            var captionName =
                NativeCaptionBookmarkName(
                    formulaId);
            var numberName =
                NativeNumberBookmarkName(
                    formulaId);
            if (!bookmarks.Exists(captionName)
            ...
            captionBookmark =
                bookmarks[captionName];
            numberBookmark =
                bookmarks[numberName];
            captionRange =
                captionBookmark.Range.Duplicate;
            numberRange =
                numberBookmark.Range.Duplicate;

            // Native #(SEQ) OMML keeps both aliases inside mathematics and is
            // handled by TryRemoveNativeHashSequenceCaptionBookmarks above.
            if (WordFormulaHost.IsWhollyInsideMath(
                    numberRange))
                return;

            numberParagraphs =
                numberRange.Paragraphs;
            if (numberParagraphs.Count != 1)
                throw new InvalidDataException(
                    "The external equation number no longer belongs to exactly one caption paragraph.");
            numberParagraph =
                numberParagraphs[1];
            numberParagraphRange =
                numberParagraph.Range.Duplicate;

            if (numberRange.StoryType !=
                    numberParagraphRange.StoryType
                || numberRange.Start <
                    numberParagraphRange.Start
                || numberRange.End >
                    numberParagraphRange.End)
                throw new InvalidDataException(
                    "The external equation number escaped its caption paragraph.");

            // Prove this is the generated caption paragraph before rebinding any
            // bookmark. It must own exactly one VisualTeX equation SEQ field and
            // that field's result must contain the stable VTEqNum target.
            paragraphFields =
                numberParagraphRange.Fields;
            var sequenceCount = 0;
            for (var index = 1;
                 index <= paragraphFields.Count;
                 index++)
            {
                Release(code);
                code = null;
                Release(field);
                field =
                    paragraphFields[index];
                code =
                    field.Code.Duplicate;
                if (!IsVisualTeXSequenceFieldCode(
                        code.Text))
                    continue;
                ...
                try
                {
                    ...
                    if (result.StoryType !=
                        ...
                        || result.End <
                            numberRange.End)
                        throw new InvalidDataException(
                            "The external equation number bookmark is not owned by its caption SEQ result.");
                }
                finally
                {
                    Release(result);
            ...
            if (sequenceCount != 1
                || matchedSequence is null)
                throw new InvalidDataException(
                    "The external equation number paragraph does not own exactly one VisualTeX SEQ field.");

            // A generated caption paragraph never owns mathematical hosts or OLEs.
            // Reject instead of trimming if the paragraph itself is contaminated;
            // only the stale outer bookmark is allowed to overlap user content.
            OMaths? maths = null;
            InlineShapes? shapes = null;
            Tables? tables = null;
            ContentControls? controls = null;
            ...
            {
                maths =
                    numberParagraphRange.OMaths;
                shapes =
                    numberParagraphRange.InlineShapes;
                tables =
                    numberParagraphRange.Tables;
                controls =
                    numberParagraphRange.ContentControls;
                if (maths.Count != 0
                    || shapes.Count != 0
                    || tables.Count != 0
                    || controls.Count != 0)
                    throw new InvalidDataException(
                        "The verified external equation caption paragraph contains unowned document content.");
            }
            finally
            {
                Release(controls);
            ...
            // Detach only the stale bookmark identity. No document text is
            // deleted here. Then bind VTEqCap_<id> to the exact paragraph proven
            // by VTEqNum_<id> + its unique SEQ field.
            captionBookmark.Delete();
            Release(captionBookmark);
            captionBookmark = null;

            reboundRange =
                numberParagraphRange.Duplicate;
            rebound =
                bookmarks.Add(
                    captionName,
                    reboundRange);
            ...
            {
                verified =
                    rebound.Range.Duplicate;
                if (verified.StoryType !=
                        numberParagraphRange.StoryType
                    || verified.Start !=
                        numberParagraphRange.Start
                    || verified.End !=
                        numberParagraphRange.End)
                    throw new InvalidDataException(
                        "Word re-expanded the repaired external equation caption bookmark.");
            }
            finally
            {
                Release(verified);
        ...
        bool preserveParagraphSeparator = false)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                if (WordFormulaHost.IsWhollyInsideMath(range))
                {
                ...
                for (var index = 1; index <= bookmarkMaths.Count; index++)
                {
                    OMath? math = null;
                    Range? mathRange = null;
                    try
                    {
                        math = bookmarkMaths[index]; mathRange = math.Range;
                        if (mathRange.StoryType == range.StoryType
                            && range.Start < mathRange.End && range.End > mathRange.Start)
                            throw new InvalidDataException("The external equation caption overlaps unowned mathematical content.");
                    }
            ...
            // Detach the owned bookmark before changing the Frame. Frame.Delete
            // can shrink its live range. Native Undo can still retain a shortened
            // span; the transaction checkpoint verifies and restores owned spans.
            var captionStart = range.Start;
            var captionEnd = range.End;
            var captionText = range.Text;
            bookmark.Delete();
            frames = range.Frames;
            if (frames.Count > 0)
            {
                if (frames.Count != 1)
                    throw new InvalidDataException("An equation caption has ambiguous Frame ownership.");
                frame = frames[1];
                Range? frameRange = null;
                try
                {
                    frameRange = frame.Range;
                    // A shared caption Frame belongs to several equations. Removing
                    // it for one source destroys the other captions' layout and can
                    // rebind their bookmarks. Only detach an exclusively owned
                    // Frame; deleting this one caption paragraph shrinks a shared
                    // Frame normally without touching its neighbours.
                    if (frameRange.Start >= captionStart && frameRange.End <= captionEnd)
                        frame.Delete();
                }
                finally { Release(frameRange); }
                Release(frame);
                ...
                frames = null;
                Release(range);
                range = document.Range(captionStart, captionEnd);
                if (range.Text != captionText)
                    throw new InvalidDataException("Removing the equation caption Frame changed its content.");
            }
        ...
        ISet<string> numberedFormulaIds)
    {
        ...
        RemoveOrphanBookmarks(
            ...
            numberedFormulaIds,
            deleteRange: false);
        // Flow-tail bookmarks own generated empty paragraphs only while their
        // numbered formula exists. Orphan cleanup drops identity rather than text;
        // destructive reclamation is performed only by the strict local teardown
        // path that can prove the exact body->table geometry.
        RemoveOrphanBookmarks(
            document,
            NativeCaptionFlowTailBookmarkPrefix,
            numberedFormulaIds,
            deleteRange: false);
    }
        ...
        string formulaId)
    {
        ...
        try
        {
            ...
            // object's height. Table numbers use the same body-font contract.
            AlignEquationNumberVertically(
                range,
                formulaHeightPoints: 0f,
    ...
    {
        numberChanged = false;
        var hasTailFormulaId = TryReadNativeEquationTailFormulaId(document, out var tailFormulaId);
        if (!hasTailFormulaId
        ...
        string formulaId)
    {
        ...
        try
        {
            ...
            for (var index = 1; index <= suffixBookmarks.Count; index++)
            {
                ...
                try
                {
                    candidate = suffixBookmarks[index];
                    if (TryFormulaIdFromBookmark(
                            candidate.Name,
                            NativeNumberBookmarkPrefix,
                            out var candidateFormulaId)
                        && !string.Equals(
                            candidateFormulaId,
                            formulaId,
                            StringComparison.OrdinalIgnoreCase))
                    {
                        hasLaterNumberOwner = true;
                        break;
                    }
                }
                finally { Release(candidate); }
    ...
    private static bool ShouldPreferScopedSequenceSuffixUpdate(Document document)
    {
        Bookmarks? bookmarks = null;
        try
        {
            bookmarks = document.Bookmarks;
            // Each healthy numbered VisualTeX formula owns at least three public
            // aliases (plus an OLE/OMML identity). This threshold keeps very small
            // documents on the simple path while avoiding repeated field walks once
            // the document reaches roughly a few dozen formulas.
            return bookmarks.Count >= 96;
        }
        catch { return true; }
        finally { Release(bookmarks); }
    }
        ...
        out int referencesAlreadyUpdatedFrom)
    {
        ...
        var format = ReadEquationNumberFormat(document);
        var scopeWatch = string.Equals(
                Environment.GetEnvironmentVariable("VISUALTEX_NUMBERED_PERF_TRACE"),
                "1",
                StringComparison.Ordinal)
            ? System.Diagnostics.Stopwatch.StartNew()
            : null;
        var scopeCheckpoint = 0L;
        void TraceScopeStage(string stage)
        {
            if (scopeWatch is null) return;
            var elapsed = scopeWatch.ElapsedMilliseconds;
            Console.WriteLine(
                $"      [perf] continuous-scope.{stage}: +{elapsed - scopeCheckpoint}ms ({elapsed}ms)");
            scopeCheckpoint = elapsed;
        }
        ...
        try
        {
            ...
            if (!TryReadHealthyEquationNumberArtifactsFromOpenXml(
                    document,
                    out var healthyEntries,
                    out var knownReferenceCounts))
                return false;
            TraceScopeStage("openxml-inventory");
            var orderedHealthyEntries = healthyEntries
                .OrderBy(entry => entry.Position)
                .ToArray();
            var currentEntryIndex = Array.FindIndex(
                orderedHealthyEntries,
                entry => string.Equals(
                    entry.FormulaId,
                    formulaId,
                    StringComparison.OrdinalIgnoreCase));
            if (currentEntryIndex < 0) return false;
            var allOrderedSuffix = orderedHealthyEntries
                .Skip(currentEntryIndex + 1)
                .ToArray();
            ...
            // Inventory the suffix once, update each affected SEQ exactly once,
            // then patch generated REF results directly. Word's Fields.Update on
            // the whole mixed OLE suffix repaginates every EMBED field even when it
            // is locked and is slower than this targeted single pass.
            var suffixHasOnlyGeneratedReferences = false;
            var suffixSequencesUpdated = !format.UsesHeading
                && changedFormulaNumbers.Count > 0
                && suffixRange is not null
                && TryUpdateHealthyVisualTeXSuffixFields(
                    suffixRange,
                    nativeSequenceName,
                    orderedSuffix,
                    changedFormulaNumbers,
            ...
            else if (!suffixSequencesUpdated)
            {
                // A legacy mathematical #(SEQ) host or an incomplete inventory
                // requires the conservative per-formula refresher.
                for (var index = 0; index < orderedSuffix.Length; index++)
                {
                    var expectedOrdinal = currentOrdinal + index + 1;
            ...
            var patchedGeneratedReferences = suffixSequencesUpdated;
            if (!patchedGeneratedReferences)
            {
                patchedGeneratedReferences = true;
            ...
            TraceScopeStage("patch-visible-results");
            if (patchedGeneratedReferences
                && suffixSequencesUpdated
                && suffixHasOnlyGeneratedReferences)
            ...
            TraceScopeStage("verify-results");
        ...
        string expectedNumber)
    {
        ...
        try
        {
            ...
            if (maths.Count == 0 && tables.Count > 0)
            {
                if (tables.Count == 1)
                {
                    table = tables[1];
                    if (TryGetManagedNumberTableRowIndex(table, visibleRange, 3, out _))
                    {
    ...
    private static bool TryUpdateHealthyVisualTeXSuffixFields(
        Range suffixRange,
        string nativeSequenceName,
        IReadOnlyList<NativeEquationCaptionEntry> orderedSuffix,
        ...
        out bool suffixHasOnlyGeneratedReferences)
    {
        ...
        Document? document = null;
        Fields? fields = null;
        var sequenceFieldCount = 0;
        var visualReferenceCount = 0;
        var externalReferenceCount = 0;
        var hasExternalReferences = false;
        var sequenceFieldOffset = -1;
        var tracePerformance = string.Equals(
            Environment.GetEnvironmentVariable("VISUALTEX_NUMBERED_PERF_TRACE"),
            "1",
            StringComparison.Ordinal);
        bool RejectInventory(string reason)
        {
            if (tracePerformance)
                Console.WriteLine($"      [perf] continuous-suffix.inventory-rejected reason={reason}");
            return false;
        }
        try
        {
            foreach (var changedFormulaId in changedFormulaNumbers.Keys)
            {
                if (!knownReferenceCounts.TryGetValue(
                        changedFormulaId,
                        out var referenceCount))
                    return RejectInventory("missing-reference-count");
                // The healthy OpenXML inventory normalizes both OLE-generated REF
                // labels and direct-table/native visible slots to a baseline count
                // of one. Anything above one proves a real body reference exists.
                if (referenceCount <= 1) continue;
                hasExternalReferences = true;
                externalReferenceCount += referenceCount - 1;
            }

            document = suffixRange.Document;
            fields = suffixRange.Fields;
            if (fields.Count == 0) return RejectInventory("no-fields");

            // Inspect and update in the same forward walk. Materializing this mixed
            // field collection a second time is itself expensive in Word, while
            // Range.Fields.Update repaginates every EMBED even when it is locked.
            // Only owned SEQ fields are refreshed; REF ownership is inventoried so
            // their already-known visible results can be patched safely afterward.
            for (var index = 1; index <= fields.Count; index++)
            {
                Field? field = null;
                Range? code = null;
                Range? sequenceResult = null;
                Range? referenceResult = null;
                try
                {
                    field = fields[index];
                    var isOwnedSequence = false;
                    if (field.Type == WdFieldType.wdFieldRef)
                    {
                        ...
                        {
                            referenceResult = field.Result;
                            if (!string.Equals(
                                    NormalizeNativeEquationNumberText(referenceResult.Text),
                                    expectedNumber,
                                    StringComparison.Ordinal))
                                referenceResult.Text = expectedNumber;
                        }
                        continue;
                    }
                    ...
                    code = field.Code;
                    var codeText = code.Text ?? string.Empty;
                    isOwnedSequence = IsVisualTeXSequenceFieldCode(codeText);

                    if (isOwnedSequence)
                    {
                        if (field.Locked) return RejectInventory("locked-sequence");
                        sequenceFieldCount++;
                        sequenceResult = field.Result;
                        if (sequenceFieldOffset < 0)
                        {
                            sequenceFieldOffset = orderedSuffix.Count > 0
                                && sequenceResult.Start < orderedSuffix[0].Position
                                ? 1
                                : 0;
                        }
                        var entryIndex = sequenceFieldCount - 1 - sequenceFieldOffset;
                        if (entryIndex < 0)
                            continue;
                        if (entryIndex >= orderedSuffix.Count)
                            return RejectInventory("unexpected-extra-sequence");

                        var entry = orderedSuffix[entryIndex];
                        if (!changedFormulaNumbers.TryGetValue(
                                entry.FormulaId,
                                out var expectedNumber))
                            continue;
                        if (!int.TryParse(
                                ...
                                System.Globalization.CultureInfo.InvariantCulture,
                                out _))
                            return RejectInventory("non-continuous-number");
                                ...
                                RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
                        {
                            if (IsFieldInsideOmml(field))
                                return RejectInventory("mathematical-sequence-host");
                            EnsureSequenceFieldCodeCanBeRewritten(field);
                            code!.Text = Regex.Replace(
                                codeText,
                                @"\\r\s+\d+",
                                "\\r " + expectedNumber,
                                RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
            ...
            // Word can expose the field whose result ends exactly at
            // suffixRange.Start as the first member of Range.Fields. Re-updating
            // that already-correct current SEQ is harmless; any other count means
            // the suffix inventory is incomplete or contains an alien sequence.
            if (sequenceFieldCount != orderedSuffix.Count
                && sequenceFieldCount != orderedSuffix.Count + 1)
                return RejectInventory(
                    $"sequence-count-{sequenceFieldCount}-expected-{orderedSuffix.Count}");
            suffixHasOnlyGeneratedReferences = !hasExternalReferences;
            if (tracePerformance)
            {
                Console.WriteLine(
                    $"      [perf] continuous-suffix.inventory fields={fields.Count} "
                    + $"sequences={sequenceFieldCount} refs={visualReferenceCount} "
                    + $"externalRefs={externalReferenceCount} sequence={nativeSequenceName}");
            }
            return true;
        }
        catch (Exception error)
        {
            if (tracePerformance)
                Console.WriteLine(
                    $"      [perf] continuous-suffix.inventory-rejected reason=exception-{error.Message}");
            suffixHasOnlyGeneratedReferences = false;
            return false;
        }
        finally
    ...
    {
        // Current numbered OMML uses a proven 1x3 direct-table SEQ. Update its
        // explicit reset ordinal directly; this also gives ordinary OLE captions a
        // cheap bookmark-topology rejection without reading CustomXML/WordOpenXML.
        if (TryUpdateDirectTableSequenceNumber(
                document,
                formulaId,
                nativeSequenceName,
                ...
                formatOnly: false,
                knownBookmarks: bookmarks,
                trustedHealthyDirectTable: true))
        {
        ...
        try
        {
            ...
            captionBookmark = bookmarks[captionName];
            captionRange = captionBookmark.Range;
            captionMaths = captionRange.OMaths;
            if (captionMaths.Count > 0)
            {
                // Retired mathematical #(SEQ) hosts are uncommon but must never
                // have Field.Code.Text rewritten in place. Their refresher performs
                // the full structural verification and atomic rebuild when needed.
                if (!TryRefreshNumberedNativeOmmlHashSequence(
                        document,
                        formulaId,
                        expectedPrefix))
                ...
                return string.Equals(
                    ...
                    expectedPrefix + expectedOrdinal.ToString(
                        System.Globalization.CultureInfo.InvariantCulture),
                    StringComparison.Ordinal);
            }
            field = FindNativeEquationFieldInRange(captionRange, nativeSequenceName);
    ...
    private static bool TryReadCompleteEquationNumberArtifactsFromBookmarks(
        Document document,
        string nativeSequenceName,
        out IReadOnlyList<NativeEquationCaptionEntry> captions)
    {
        ...
        try
        {
            ...
            var captionIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            var numberIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            bookmarks = document.Bookmarks;
            for (var index = 1; index <= bookmarks.Count; index++)
            {
                Release(bookmark);
                bookmark = bookmarks[index];
                var name = bookmark.Name ?? string.Empty;
                if (TryFormulaIdFromBookmark(name, NativeCaptionBookmarkPrefix, out var formulaId))
                {
                    captionIds.Add(formulaId);
                    continue;
                }
                if (TryFormulaIdFromBookmark(name, NativeNumberBookmarkPrefix, out formulaId))
                {
                    numberIds.Add(formulaId);
                    continue;
                }
                if (TryFormulaIdFromBookmark(name, EquationBookmarkPrefix, out formulaId))
                {
                    visibleIds.Add(formulaId);
                    continue;
                }
                if (name.StartsWith(NativeDisplayAnchorBookmarkPrefix, StringComparison.OrdinalIgnoreCase)
                    || name.StartsWith(NativeDisplayAnchorCommitBookmarkPrefix, StringComparison.OrdinalIgnoreCase))
                    return false;
            }

            if (visibleIds.Count == 0
                || !visibleIds.SetEquals(captionIds)
                || !visibleIds.SetEquals(numberIds))
                return false;

            var resolved = GetNativeEquationCaptionEntries(document, nativeSequenceName);
            var resolvedIds = resolved
                .Select(item => item.FormulaId)
                .ToHashSet(StringComparer.OrdinalIgnoreCase);
            if (resolved.Count != visibleIds.Count || !resolvedIds.SetEquals(visibleIds))
                return false;
            captions = resolved;
            return true;
        }
            ...
            for (var index = 1; index <= bookmarks.Count; index++)
            {
                ...
                try
                {
                    ...
                    if (!bookmarks.Exists(NativeCaptionBookmarkName(formulaId))
                        ...
                        continue;
                    // The fallback must use the same owner order as the XML path.
                    // Resolve named bookmarks only; do not activate embedded OLE.
                    numberRange = bookmark.Range;
                    visibleBookmark = bookmarks[EquationBookmarkName(formulaId)];
                    visibleRange = visibleBookmark.Range;
                    result.Add(new NativeEquationCaptionEntry(
                        formulaId,
                        visibleRange.Start,
        ...
        bool trustedHealthyDirectTable = false)
    {
        ...
        try
        {
            ...
            if (!formatOnly)
            {
                ...
                // The strict XML inventory already proved this existing host.
                // Updating only its ordinal/prefix must not reapply Frame/font/
                // paragraph styles for every formula; that layout work dominated
                // a 50-number refresh. Malformed/legacy paths retain full restyling.
                if (!trustedHealthyDirectTable)
                    StyleNativeCaption(
                        captionRange,
        ...
        Func<string?, bool> predicate)
    {
        ...
        try
        {
            ...
            ApplyParagraphEquationNumberFont(range, fallbackSize: 0f, position: 0);
        }
        finally
        {
            Release(fields);
        ...
        float formulaFontSizePoints)
    {
        ...
        ApplyParagraphEquationNumberFont(
            numberRange,
            numberFontSize,
            CalculateEquationNumberFontPosition(formulaHeightPoints, numberFontSize));
    }
    ...
    private static void ApplyParagraphEquationNumberFont(
        ...
        int position)
    {
        // Word can transiently expand a just-created REF/bookmark range across
        // the following paragraph while a batch of numbered formulas is being
        // materialized. Formatting that expanded range both throws in the strict
        // paragraph-mark helper and can leak the number style into following body
        // text. Resolve the paragraph that owns the label's first character and
        // clamp the formatting write to that paragraph only.
        Paragraphs? paragraphs = null;
        Paragraph? paragraph = null;
        Range? paragraphRange = null;
        ...
        try
        {
            ...
            if (paragraphs.Count == 0)
                throw new InvalidDataException("The equation number has no paragraph owner.");
            paragraph = paragraphs[1];
            paragraphRange = paragraph.Range;
            var bodyFormatting = WordCharacterFormatting.CaptureParagraphMark(paragraphRange);

            effectiveRange = targetRange.Duplicate;
            var start = Math.Max(effectiveRange.Start, paragraphRange.Start);
            var end = Math.Min(effectiveRange.End, paragraphRange.End);
            if (end <= start)
                throw new InvalidDataException("The equation number range does not intersect its paragraph owner.");
            effectiveRange.SetRange(start, end);

            font = effectiveRange.Font;
            font.Reset();
            if (fallbackSize > 0f)
                font.Size = FormulaFontSize.Normalize(fallbackSize);
            bodyFormatting.Apply(font);
            font.Position = position;
        }
        finally
        {
            Release(font);
            Release(effectiveRange);
            Release(paragraphRange);
            Release(paragraph);
            Release(paragraphs);
        }
        ...
        IReadOnlyDictionary<string, int>? knownReferenceCounts = null)
    {
        ...
        Fields? fields = null;
        var updated = 0;
        var foundTargetReference = false;
        try
        {
            ...
            for (var index = 1; index <= fields.Count; index++)
            {
                ...
                try
                {
                    ...
                    // Body references (including nested REF entries exposed by
                    // Document.Fields) belong to the common formatting-preserving
                    // refresher. Only a physical generated number is patched here.
                    if (!IsGeneratedEquationNumberReference(document, field, formulaId))
                        continue;
        ...
        // In the scoped middle-insertion path the suffix's generated labels were
        // already patched directly. If the prefix contains no REF to a shifted
        // formula there is nothing left for the document-wide reference walker to
        // do; avoiding that scan is material in documents with hundreds of EMBEDs.
        if (searchEndExclusive.HasValue && !foundTargetReference)
            return updated;
        return updated + WordEquationReferenceFields.UpdateReferences(document,
            changedFormulaNumbers.Keys.Select(NativeNumberBookmarkName)
                .ToHashSet(StringComparer.OrdinalIgnoreCase));
    }
    ...
    internal static int UpdateNativeCrossReferences(
        Document document,
        ISet<string>? targetFormulaIds)
    {
        ...
        try
        {
            ...
            for (var index = 1; index <= fields.Count; index++)
            {
                ...
                try
                {
                    ...
                    // A body REF keeps its original bookmark and delimiter contract.
                    // Resolving an alias to an equation identifies its owner; it does
                    // not authorize rewriting ZEqnNum (visible label) to VTEqNum
                    // (digits only). Canonicalize only the generated number itself.
                    if (!IsGeneratedEquationNumberReference(document, field, formulaId))
                        continue;
        ...
        // One common body-reference pass handles both direct and nested REF.
        ISet<string>? targetBookmarkNames = targetFormulaIds is null
            ? null
            : targetFormulaIds
                .Select(NativeNumberBookmarkName)
                .ToHashSet(StringComparer.OrdinalIgnoreCase);
        updated += WordEquationReferenceFields.UpdateReferences(
            document,
            targetBookmarkNames);
        return updated;
    }
    ...
    internal static bool IsGeneratedEquationNumberReference(
        Document document, Field field, string formulaId)
    {
        Range? label = null;
        Range? owner = null;
        Range? code = null;
        ...
        try
        {
            ...
            label = FindVisibleEquationNumberRange(document, formulaId);
            if (label is null) return false;
            owner = FindNumberingOwnerRange(document, formulaId);
            ...
            // Word 2021 may report Result.End at or beyond the paragraph
            // structure position for a REF whose visible digits are still wholly
            // inside VTEq_*. End is therefore not a stable identity boundary.
            // Prove generation from the local owner and the result's start
            // instead: this exact REF targets this formula's private VTEqNum_*,
            // both code and result live in the numbering owner, and the visible
            // result starts inside the VTEq_* label. A user/body REF to the same
            // bookmark lives outside that private label and does not satisfy this.
            return code.StoryType == owner.StoryType
                && result.StoryType == owner.StoryType
                && label.StoryType == owner.StoryType
                && code.Start >= owner.Start
                && code.Start < owner.End
                && result.Start >= label.Start
                && result.Start <= label.End
                && result.Start >= owner.Start
                && result.Start < owner.End;
        }
        catch
        {
            return false;
        ...
        float? knownFontSize = null)
    {
        ...
        try
        {
            ...
            // CHARFORMAT protects the visible number from its clipped SEQ
            // target. Creation and refresh share the body's paragraph-mark format
            // for table, tab and text-frame hosts alike.
    ...
    internal static string NativeCaptionFlowTailBookmarkName(string formulaId) =>
        BookmarkName(NativeCaptionFlowTailBookmarkPrefix, formulaId);
